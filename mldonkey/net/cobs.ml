...
let decoded_length psrc srclen =
  let rec iter codepos len =
    if codepos < srclen then
      let code = psrc.[codepos] in
      let code = int_of_char code in
      iter (codepos + code) (len + code - if code = 255 then 1 else 0)
    else len - 1
  in
  iter 0 0

let decode_data pdest psrc srclen =
  let rec iter srcpos dstpos =
    if srcpos < srclen then (
      ...
      iter srcpos
        ( if code < 255 && srcpos < srclen then (
...
let encoded_length psrc srclen =
  let rec iter srcpos len code =
    if srcpos < srclen then
      let c = psrc.[srcpos] in
      let srcpos = srcpos + 1 in
      if c = '\000' then iter srcpos (len + code) 1
      else
        let code = code + 1 in
        if code = 255 then iter srcpos (len + code) 1 else iter srcpos len code
    else len + code
  in
  iter 0 0 1

let encode_data pdest psrc srclen =
    ...
    if srcpos < srclen then (
      ...
      if c = '\000' then (
        ...
        if code = 255 then (
          ...
          let dstpos = dstpos + 1 in
          iter dstpos srcpos codepos 1 )
        else iter dstpos srcpos codepos code )
    else Bytes.set pdest codepos @@ char_of_int code
  in
  iter 1 0 0 1

let decode psrc =
  let srclen = String.length psrc in
  let dstlen = decoded_length psrc srclen in
  let pdest = Bytes.create dstlen in
  decode_data pdest psrc srclen;
  Bytes.unsafe_to_string pdest

let encode psrc =
  let srclen = String.length psrc in
  let dstlen = encoded_length psrc srclen in
  let pdest = Bytes.create dstlen in
  encode_data pdest psrc srclen;
  Bytes.unsafe_to_string pdest

let%test _ = encode "" = "\x01"

let%test _ = "" = decode "\x01"

let%test _ = encode "1" = "\x021"

let%test _ = "1" = decode "\x021"

let%test _ = encode "12345" = "\x0612345"

let%test _ = "12345" = decode "\x0612345"

let%test _ = encode "12345\x006789" = "\x0612345\x056789"

let%test _ = "12345\x006789" = decode "\x0612345\x056789"

let%test _ = encode "\x0012345\x006789" = "\x01\x0612345\x056789"

let%test _ = "\x0012345\x006789" = decode "\x01\x0612345\x056789"

let%test _ = encode "12345\x006789\x00" = "\x0612345\x056789\x01"

let%test _ = "12345\x006789\x00" = decode "\x0612345\x056789\x01"

let%test _ = encode "\x00" = "\x01\x01"

let%test _ = "\x00" = decode "\x01\x01"

let%test _ = encode "\x00\x00" = "\x01\x01\x01"

let%test _ = "\x00\x00" = decode "\x01\x01\x01"

let%test _ = encode "\x00\x00\x00" = "\x01\x01\x01\x01"

let%test _ = "\x00\x00\x00" = decode "\x01\x01\x01\x01"

let%test _ =
  encode "Hello world\x00This is a test" = "\x0cHello world\x0fThis is a test"

let%test _ =
  "Hello world\x00This is a test" = decode "\x0cHello world\x0fThis is a test"

let%test _ =
  let s = String.make 253 'a' in
  encode s = "\254" ^ s

let%test _ =
  let s = String.make 254 'a' in
  encode s = "\255" ^ s ^ "\001"
...
(* FIXME: the decoder should tolerate this fault
let%test _ =
  let s = String.make 254 'a' in
  decode ("\255" ^ s) = s *)
...
module GGEP = struct
  type ggep_block = UnknownBlock of string | GGEP of string * string
  ...
  let rec get_len s pos len =
    ...
    if again then get_len s (pos + 1) len else (len, pos + 1)
  ...
  let rec iter_block s pos len list =
    if pos = len then list
      ...
      let data = if cobs_encoded then decode data else data in
...
let parse s =
  ...
  let print_block s =
    Printf2.lprintf "New block\n";
    let list = parse s in
    List.iter
      (fun b ->
        match b with
        | UnknownBlock s ->
            Printf2.lprintf "Unknown Block: %s\n" (String.escaped s)
        | GGEP (id, data) ->
            Printf2.lprintf "GGEP Block:\n";
...
let _ =
  ...
  *)
  ...
  let write_ggep buf put_magic last_block b =
    match b with
    | UnknownBlock s -> Buffer.add_string buf s
    | GGEP (id, data) ->
    ...
    List.map
      (fun x ->
        ...
        | GGEP_VC_vendor (_vendor, version1, version2) ->
            let s =
              Printf.sprintf "MLDK%c"
                (char_of_int ((version1 lsl 4) lor version2))
            in
            GGEP.GGEP ("VC", s)
        | GGEP_DU_uptime up ->
            let s =
              if up land 0xff = up then String.make 1 (char_of_int up)
              else if up land 0xffff = up then (
...
let print list =
  ...
  List.iter
    (fun x ->
      match x with
      | GGEP_block (id, data) ->
          Printf2.lprintf "   Block %s : %s\n" id (String.escaped data)
      | GGEP_unknown s -> Printf2.lprintf "   Unknown : %s\n" (String.escaped s)
      | GGEP_GUE_guess v -> Printf2.lprintf "   Guess version %d\n" v
      | GGEP_VC_vendor (vendor, v1, v2) ->
          Printf2.lprintf "   Vendor %s %d.%d\n" vendor v1 v2
      | GGEP_DU_uptime up -> Printf2.lprintf "   Uptime %d seconds\n" up)
    list
