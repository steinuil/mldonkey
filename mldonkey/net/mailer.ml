...
let hmac_md5 =
  ...
  fun secret challenge ->
    ...
    md5 (string_xor (Bytes.unsafe_to_string k) opad ^ md5 (string_xor (Bytes.unsafe_to_string k) ipad ^ challenge))
    ...
    try
      ...
      begin
        ...
        begin
          ...
          | (334,true,s) ->
            (* RFC 2195 *)
            let digest = hmac_md5 mail.smtp_password (Base64.decode_exn s) in
            send oc (Base64.encode_exn (Printf.sprintf "%s %s" mail.smtp_login digest));
        ...
        begin
          ...
          send oc (Base64.encode_exn mail.smtp_login);
          if read_response ic <> 334 then bad_response (); 

          send oc (Base64.encode_exn mail.smtp_password);
          if read_response ic <> 235 then bad_response ()
        end
        else if !auth_plain_enabled then
        begin
          let auth = Printf.sprintf "\x00%s\x00%s" mail.smtp_login mail.smtp_password in
          send1 oc "AUTH PLAIN" (Base64.encode_exn auth);
          if read_response ic <> 235 then bad_response ()
        end
      end;
