...
(*      let kind = get_uint8 s (pos+22) in *)
    ...
    let write buf t =
      ...
      | OvernetGetSearchResults (md4, _kind, min, max) ->
          buf_int8 buf 16;
          buf_md4 buf md4;
...
(*          buf_int8 buf kind; *)
      ...
      | OvernetSearchSourcesResults (_md4, [_p]) ->
(*          buf_int8 buf 17;
          buf_md4 buf md4;
          buf_md4 buf r_md4;
          buf_tags buf r_tags names_of_tag
...
(*	    let kind = get_uint8 s 22 in *)
        ...
        | 11 ->
            let peers, _pos = get_list16 get_peer s 0 in
            OvernetConnectReply peers
        | 12 ->
            let md4 = get_md4 s 0 in
            let ip = get_ip s 16 in
...
(*	    let kind = get_uint8 s 22 in *)
        ...
        | 15 ->
            let md4 = get_md4 s 0 in
            let peers, _pos = get_list8 get_peer s 16 in
            OvernetSearchReply (md4, peers)
        | 16 ->
            let md4 = get_md4 s 0 in
        ...
        | 17 ->
            ...
            let r_tags, _pos = get_tags s 32 names_of_tag in
