...
(*      lprintf "port: %d\n" port;*)
      let tags, _pos = DonkeyMftp.get_tags s 23 names_of_tag in
      {
        md4 = md4;
        ip = ip;
        port = port;
...
module QueryServerList = struct
    ...
    let write (_buf: Buffer.t) (_t: t) = unit
...
module Share = struct
    ...
    let parse _len s =
      ...
      let files, _pos = get_files s 5 n in
      files
...
module ServerInfo = struct
    ...
    let parse _len s =
      ...
      let port = get_port s 21 in
(*      lprintf "port: %d\n" port; *)
      let tags, _pos = get_tags s 23 names_of_tag in
      {
        md4 = md4;
        ip = ip;
        port = port;
...
module QueryReply  = struct
    ...
    let get_replies s pos =
      ...
      let files, _pos = get_files s (pos+4) n in
      files

    let parse _len s = get_replies s 1
...
translation, i.e. Field_Artist = "Artist" instead of "artist" *)
    ...
    let parse _len s =
      let t, _pos = parse_query s 1 in t
...
module QueryUsers = struct (* request 26 *)
    ...
    let parse _len s =
      ...
      match targ with
        4 -> ""
      | 1 ->
          let name, _pos = get_string s 2 in
          name
      | _ ->
...
module Req = struct
    ...
    let parse _len _s = raise Not_found
    let print _t = raise Not_found
    let write _buf _s = raise Not_found
  end
...
let mldonkey_extensions _len s =
  check_string s 1;
  ...
  | 2 ->
      ...
      let query, _pos = Query.parse_query s 10 in
...
(* MLDONKEY *)
    ...
    | 0xD4 -> (* 212 *)
          let s = Zlib.uncompress_string2 (Bytes.unsafe_of_string (String.sub s 1 (len-1))) in
