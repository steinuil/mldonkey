...
let print_bitmap chunks =
  lprintf "\n%s\n" (Bytes.unsafe_to_string (Bitv.to_bytes chunks))
...
module QueryChunksReply = struct (* Request 80 *)
    ...
    let parse _len s =
      let md4 = get_md4 s 1 in
      let chunks, _pos = get_bitmap s 17 in
      {
        md4 = md4;
        chunks = chunks;
      }
    ...
    let print t =
      ...
      lprintf_nl "%s\n" (Bytes.unsafe_to_string (Bitv.to_bytes t.chunks))
...
module ViewDirsReply = struct
    ...
    let parse _len s =
      let dirs, _pos = get_dirs s 2 (get_int16 s 0) in
      dirs
...
module ViewFilesDir = struct
    ...
    let parse _len s =
      let dir, _pos = get_string s 1 in
      dir
...
module ViewFilesDirReply = struct
    ...
    let parse _len s =
      ...
      let files, _pos = ViewFilesReply.get_files s (pos+5) n in
      dir, files
...
module EmuleQueueRanking = struct
    ...
    let parse _len s = get_int16 s 1
    let print t =
