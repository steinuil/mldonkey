...
module ServerDescUdp = struct
  ...
  let parse _len s = Int64.of_string s
...
module ServerDescReplyUdp = struct
  ...
  let parse1 _len s challenge =
    let name, pos = get_string s 1 in
    let desc, _pos = get_string s pos in
     {
       tags = [];
       name = name;
       desc = desc;
  ...
  let parse2 _len s challenge =
    let stags,_pos = get_tags s 5 names_of_tag in
    let name = ref "" in
    let desc = ref "" in
    List.iter (fun tag ->
      match tag with
