  ...
  match t with
      M.SetIDReq t ->
        ...
        (match s.server_obfuscation_tcp with
          ...
          | Some _p -> if not t.M.SetID.tcp_obfuscation then s.server_obfuscation_tcp <- None);
        (match s.server_obfuscation_udp with
          | None -> if t.M.SetID.udp_obfuscation then s.server_obfuscation_udp <- Some 0
          | Some _p -> if not t.M.SetID.udp_obfuscation then s.server_obfuscation_udp <- None);
...
let update_master_servers _ =
  ...
  let make_master s =
    ...
    do_if_connected s.server_sock (fun _sock ->
