...
let remove_current_file file =
  current_files := List.filter (fun (f,_start,_last) ->
...
(* taken from eMule0.47c-Sources/srchybrid/kademlia/kademlia/Defines.h  *)
      ...
      search_start = (match kind with
                       KeywordSearch _s -> last_time ()
                     | FillBuckets -> last_time ()
                     | FileSearch _s -> starttime);
...
let ip_of_udp_packet p =
  match p.UdpSocket.udp_addr with
    Unix.ADDR_INET (inet, _port) ->
      Ip.of_inet_addr inet
  | _ -> assert false

let port_of_udp_packet p =
  match p.UdpSocket.udp_addr with
    Unix.ADDR_INET (_inet, port) -> port
  | _ -> assert false
...
let udp_client_handler t p =
  ...
  | OvernetPublicized (Some _p) ->
      ()

  | OvernetNoResult _ ->
      ()

  | OvernetSearch (nresults, md4, _from_who) ->
...
*)
    ...
    "md4", Arg_none (fun _o -> "MD4 is " ^ (Md4.to_string !!overnet_md4);
    ...
    "send_udp", Arg_three (fun ip port hex _o ->
        try
...
let _ =
  CommonWeb.add_web_kind web_info web_info_descr (fun _url filename ->
    try
