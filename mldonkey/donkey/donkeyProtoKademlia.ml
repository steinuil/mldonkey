...
(* Strange: why was the IP format changed for Kademlia ? *)
    ...
    let write buf t =
      ...
      | OvernetGetSearchResults (target, kind, _min, _max) ->
          buf_int8 buf 0x30;
          buf_md4 buf target;
          begin
            match kind with
...
(*	  let n = get_int16 s 0 in *)
      ...
      | 0x38 ->
          let target = get_md4 s 0 in
          let answers, _pos = get_list16 (fun s pos ->
