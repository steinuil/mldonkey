...
(*  DonkeySources.reschedule_sources file; *)
  List.iter (fun s ->
      ...
      | Connection _sock, (Connected _ | Connected_downloading _) ->
    ...
    List.iter (fun r ->
        try
          ...
          List.iter (fun tag ->
              ...
              |  { tag_name = Field_KNOWN "country" ; tag_value = String _s } -> ()
              |  { tag_name = Field_KNOWN "udpflags" ; tag_value = Uint64  _s } -> ()
              |  { tag_name = Field_KNOWN "refs" ; tag_value = Uint64  _s } -> ()
...
let unpack_server_met filename url =
  let ext = String.lowercase_ascii (Filename2.extension filename) in
    ...
    | ".met.gz" | ".met.bz2" | ".gz" | ".bz2" ->
        ...
        try
          Misc2.archive_extract filename filetype
        with
...
*)
    ...
    "import_temp", Arg_one (fun dirname _o ->
        try
          import_temp dirname;
          "temp files loaded"
        with e ->
...
*)
    ...
    "servers", Arg_one (fun filename _o ->
  if !!update_server_list_server_met then
    begin
      let nservers = List.length (Hashtbl2.to_list servers_by_key) in
  ...
  client_ops.op_client_info <- (fun c ->
      ...
      { (impl_client_info c.client_client) with
        ...
        P.client_kind = (match c.client_kind with
          ...
          | Indirect_address (_server_ip, _server_port, _ip, port, real_ip) -> 
...
\\</script\\>" (file_num file)
  ...
  file_ops.op_file_print_sources <- (fun file o ->
    ...
    begin
          ...
          html_mods_td buf ([
            ("", "sr", (match c.client_download with
                ...
                | Some _ -> (
                      ...
                      let (qfile, _qchunks,_) =  List.hd qfiles in
                      if (qfile == file) then
                        "A" else "";)) );
      ...
      ) (List.sort (fun (_s1,c1) (_s2,c2) -> compare (client_num c1) (client_num c2)) !sources_list);
...
let _ =
  ...
  client_ops.op_client_browse <- (fun c _immediate ->
      if !verbose then lprintf_nl "connecting friend %s" (full_client_identifier c);
      match c.client_source.DonkeySources.source_sock with
      | Connection _sock when c.client_emule_proto.emule_noviewshared <> 1 ->
  ...
  client_ops.op_client_print_info <- (fun c o ->
      let buf = o.conn_buf in
      let _ip_string,cc,cn = get_ips_cc_cn c in
      ...
      Printf.bprintf buf "  kind: %s\n" (
        ...
        | Indirect_address (server_ip, server_port, _id, port, real_ip) ->
                    Printf.sprintf "lowID %s:%d, server %s:%d"
                      (Ip.to_string real_ip) port (Ip.to_string server_ip) server_port
        | Invalid_address (_name,_md4) -> Printf.sprintf "invalid"
        );
        ...
        (match c.client_download with
          ...
          | Some _ ->  (
            ...
            Printf.bprintf buf "[Donkey%6d] Name  : %-27s IP   : %-20s"
              ...
              (match c.client_kind with
                  Direct_address (ip,_port) -> (Ip.to_string ip)
  ...
  client_ops.op_client_dprint_html <- (fun c o file str ->
      ...
      onClick=\\\"parent.fstatus.location.href='submit?q=friend_add+%d'\\\"\\>%d\\</TD\\>"
                    ...
                    html_mods_td buf ([
                      ...
                      ("", "sr", (match c.client_kind with
                            | Direct_address (_ip,_port) -> Printf.sprintf "D"
                            | _ -> Printf.sprintf "I"
                          ));
                      ("", "sr", (match c.client_sui_verified with
                        | None -> "N"
