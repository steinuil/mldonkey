...
let (_shared_ops : file CommonShared.shared_ops) =
    ...
    with e ->
      ...
      (match file_state with
        ...
        | _ ->
            ...
            CommonSwarming.set_verified swarmer (fun nblocks _num ->
...
let find_server ip _port =
  let key = (ip) in
  Hashtbl.find servers_by_key key

let remove_server ip _port =
  let key = (ip) in
...
let create_client key cc =
  ...
  let s = DonkeySources.create_source_by_uid (match key with
      Indirect_address (server_ip, server_port, id, _port, _real_ip) -> Indirect_address (server_ip, server_port, id, 0, Ip.null) 
      | _ -> key) cc in
  let rec c = {
      client_client = client_impl;
      client_kind = key;
...
let clean_join_queue_tables () =
  ...
  List.iter (fun (key, ((_v,time) as e)) ->
...
let _ =
  option_hook client_private_key (fun _ ->
    if Autoconf.donkey_sui_works () then
      begin
        if not (try String.sub !!client_private_key 0 4 = "MIIB" with _e -> false) then
