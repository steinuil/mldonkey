...
(* clean_client_zones: clean all structures related to downloads when
   a client disconnects *)
          ...
          List.iter (fun (_file, _chunks, up) -> 
...
let to_lowercase s = String.lowercase_ascii s
...
let update_client_from_tags c tags =
  ...
  List.iter (fun tag ->
      ...
      | Field_KNOWN "emule_udpports" -> 
          for_two_int16_tag tag (fun _ed2k_port kad_port ->
...
let send_pending_messages c _sock =
  let module M = DonkeyProtoClient in
...
let client_to_client _for_files c t sock = 
  let module M = DonkeyProtoClient in
...
(* Upload requests *)
  | M.ViewFilesReq _t when !CommonGlobals.has_upload = 0 && 
...
(*
  ...
  | M.ViewDirsReq _t when !CommonGlobals.has_upload = 0 && 
    (match !!allow_browse_share with
        1 -> client_friend_tag land client_type c <> 0
      | 2 -> true
      | _ -> false) -> 
...
(*      !say_hook c s *)
  ...
  | M.EmuleCaptchaReq t ->
      let b64data = Base64.encode_exn t in
      let cip = string_of_client_addr c in
...
let client_handler2 c sock event = 
  (match event with
      BASIC_EVENT (CLOSED _s) -> decr DonkeySources.indirect_connections
    | _ -> ());
  match !c with
...
let init_client sock c =
  set_handler sock WRITE_DONE (fun _s ->
...
let matches_3 l ip =
  let rec iter l (a,b,c,d) =
    match l with
      [] -> Ip.null
    | ip :: _ when 
      let (w,x,y,_z) = Ip.to_ints ip in
      w=a && x=b && y=c -> ip
    | _ :: t -> iter t (a,b,c,d)
  in
  iter l (Ip.to_ints ip)

let client_connection_handler overnet _t event =
  match event with
      ...
      try
        match s_uid with
          Direct_address _ ->
            ...
            reconnect_client c
        | Invalid_address _ -> ()
        | Indirect_address (server_ip, server_port, id, _port, _real_ip) ->
