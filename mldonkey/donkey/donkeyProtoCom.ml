  ...
  fun sock _nread ->
    ...
    try
      while b.len >= 5 do
        ...
        in
        let buf_str = Bytes.to_string b.buf in
        let opcode = get_uint8 buf_str b.pos in
        let msg_len = get_int buf_str (b.pos+1) in
...
let cut_messages parse f sock _nread =
  let b = TcpBufferedSocket.buf sock in
  try
    while b.len >= 5 do
      let buf_str = Bytes.to_string b.buf in
