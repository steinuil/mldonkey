...
let parse filename f =
  match%bitstring bitstring_of_file filename with
  | {|0l : 32; 02l : 32 : littleendian; count : 32 : littleendian, bind(Int32.to_int count); 
    ...
    ->
      let () = for i = 0 to pred count do
        (match%bitstring subbitstring nodes (34 * 8 * i) (34 * 8) with
        | {|id : 16 * 8 : string;
  ...
  | {| count : 32 : littleendian, bind(Int32.to_int count);
    ...
    ->
      let () = for i = 0 to pred count do
        (match%bitstring subbitstring nodes (25 * 8 * i) (25 * 8) with
        | {|id : 16 * 8 : string;
            ip1 : 8; ip2 : 8; ip3 : 8; ip4 : 8; (* littleendian *)
            udp : 16 : littleendian;
            tcp : 16 : littleendian;
          ...
          |} ->
            ...
            f ip udp
        | {| _ |} -> failwith "v0: bad contact"
        )
      done in ()
  | {| _ |} -> failwith "nodes.dat bad header"
