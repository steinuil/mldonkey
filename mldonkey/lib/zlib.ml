...
external deflate:
  stream -> bytes (* not mutated *) -> int -> int -> bytes -> int -> int -> flush_command
         -> bool * int * int
...
external update_crc: int32 -> bytes (* not mutated *) -> int -> int -> int32
...
let gzip_string ?(level = 6) inbuf =
  ...
  let rec compr inpos outbuf outpos =
    ...
    then compr inpos (grow_buffer outbuf) outpos
      ...
      let (finished, used_in, used_out) =
        deflate zs (Bytes.unsafe_of_string inbuf) inpos inavail outbuf outpos outavail
                   (if inavail = 0 then Z_FINISH else Z_NO_FLUSH) in
         out_crc := update_crc !out_crc (Bytes.unsafe_of_string inbuf) inpos used_in;
      if finished then 
        Bytes.sub outbuf 0 (outpos + used_out)
      else
        compr (inpos + used_in) outbuf (outpos + used_out)
