...
let create_file t _file file_entry_size = 
...
let save t doc v attr =  
  ...
  let file = try
    ...
    with Not_found ->
        ...
        file
  in
  let pos = file_store file (Bytes.of_string str) in
  if verbose then begin
...
let get t doc = 
  ...
  let pos, chunk_size, _attrib = uncombine combine in
...
let remove t doc = 
  let combine = t.store_all_doc.(doc) in
  let pos, chunk_size, _attr = uncombine combine in
  let file = List.assoc chunk_size t.store_files in
