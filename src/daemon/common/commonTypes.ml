...
and file_uid_id = BITPRINT | SHA1 | ED2K | MD5 | MD5EXT | TIGER | FILETP
...
let string_of_uid_sep uid sep =
  ...
  | Sha1 sha1 -> "urn" ^ sep ^ "sha1" ^ sep ^ Sha1.to_string sha1
  | Ed2k ed2k -> "urn" ^ sep ^ "ed2k" ^ sep ^ Md4.to_string ed2k
  | Md5 md5 -> "urn" ^ sep ^ "md5" ^ sep ^ Md5.to_string md5
  | TigerTree ttr ->
      "urn" ^ sep ^ "tree" ^ sep ^ "tiger" ^ sep ^ TigerTree.to_string ttr
  | Md5Ext md5 -> "urn" ^ sep ^ "sig2dat" ^ sep ^ Md5Ext.to_base32 md5
  | BTUrl url -> "urn" ^ sep ^ "btih" ^ sep ^ Sha1.to_string url
  | FileTP file -> "urn" ^ sep ^ "filetp" ^ sep ^ Md4.to_string file
  | NoUid -> ""

let string_of_uid uid = string_of_uid_sep uid ":"

(* This should be used for temp. files, because for some
   systems the character ":" may be illegal for filenames.
 *)
let file_string_of_uid uid = string_of_uid_sep uid "_"
...
let uid_of_string s =
  ...
  | "bt" | "btih" ->
      ...
      BTUrl
        (if String.length rem = 40 then Sha1.of_hexa rem else Sha1.of_string rem)
...
module Uid : sig
  ...
  type t = { mutable uid_string : string; mutable uid_type : uid_type }

  let no = { uid_type = NoUid; uid_string = "" }

  let create uid = { uid_string = string_of_uid uid; uid_type = uid }

  let of_string s =
    let uid = uid_of_string s in
    let s = string_of_uid uid in
    { uid_string = s; uid_type = uid }

  let to_string t = t.uid_string

  let to_file_string t = file_string_of_uid t.uid_type

  let to_uid t = t.uid_type

  let derive uid =
    match uid.uid_type with
    | Bitprint (sha1, tiger) -> [ create (Sha1 sha1); create (TigerTree tiger) ]
    | _ -> []

  let compare u1 u2 = compare (to_string u1) (to_string u2)

  let rec mem uid list =
    match list with
    | [] -> false
    | t :: tail -> compare t uid = 0 || mem uid tail

  let add uid list = if not (mem uid list) then uid :: list else list
...
let parse_magnet_url url =
  ...
  if url.Url.short_file = "magnet:" then (
    ...
    let each k v =
      ...
      | "mt" :: _ ->
          ()
          (* manifest topic: url or urn, see http://rakjar.de/gnuticles/MAGMA-Specsv22.txt *)
      | "kt" :: _ -> () (* keywords topic *)
...
*)
    ...
    List.iter
      (fun (k, v) ->
        try each k v
        with exn ->
          lprintf_nl "MAGNET: field %S=%S, exn %s" k v (Printexc2.to_string exn))
      url.Url.args;
    object
      method name = !name
...
let show_magnet_url x =
  let args =
    ("dn", x#name) :: List.map (fun uid -> ("xt", string_of_uid uid)) x#uids
  in
  let args =
    match x#size with
    | Some n -> ("xl", Int64.to_string n) :: args
    | None -> args
  in
...
let string_of_uids list =
  match list with [] -> "<unknown>" | uid :: _ -> Uid.to_string uid
...
type tag = { mutable tag_name : field; mutable tag_value : tag_value }
...
type query =
  ...
  | QNone
      (** temporary, used when no value is available ;
...
type compressor = Deflate of Zlib.stream * Zlib.stream
...
type tcp_connection =
  ...
  | Connection of TcpBufferedSocket.t
...
type room_state = RoomOpened | RoomClosed | RoomPaused
...
type search_type = RemoteSearch | LocalSearch | SubscribeSearch
...
type extend_search = ExtendSearchLocally | ExtendSearchRemotely
...
type groupdb = { group_name : string; mutable group_admin : bool }
...
and network = {
  ...
  mutable op_network_add_server : Ip.addr -> int -> server;
  ...
  mutable op_network_share : string -> string -> int64 -> unit;
  mutable op_network_private_message : string -> string -> unit;
  mutable op_network_parse_url :
    string -> userdb -> groupdb option -> string * bool;
  ...
  mutable op_network_stat_info_list :
    unit -> (string * int * network_stat_info list) list;
...
and search = {
  ...
  (* how many replies are we waiting for *)
  mutable search_string : string;
  mutable search_closed : bool;
  (* should we continue to ask/wait for results *)
...
(* as per OggDS new Header Format      *)
...
type theora_color_space = CSUndefined | CSRec470M | CSRec470BG
...
type ogg_tag =
  ...
  | Ogg_theora_avgbytespersec of int

(* in bytes/s for VBR streams          *)
...
let ogg_tag_to_string ogg_tag =
  ...
  | Ogg_vorbis_blocksize_0 n ->
      Printf.sprintf "blocksize_0: %d" (int_of_float (2. ** float_of_int n))
  | Ogg_vorbis_blocksize_1 n ->
      Printf.sprintf "blocksize_1: %d" (int_of_float (2. ** float_of_int n))
  ...
  List.iter
    (fun stream ->
      lprintf "-== %s info - stream number: %d ==-\n"
        (stream_type_to_string stream.stream_type)
        stream.stream_no;
      List.iter
...
let is_connected state =
  match state with
  | Connected_initiating | Connected_downloading _ | Connected _ -> true
  | NotConnected _ | Connecting | NewHost | BlackListedHost | ServerFull
  | RemovedHost ->
      false
...
let string_of_connection_state s =
  ...
  | NotConnected (_, n) ->
      if n = -1 then ""
      else if n = 0 then "Queued Out"
      else if n > 0 then Printf.sprintf "Ranked %d Out" n
      else Printf.sprintf "Failed %d" (-n - 1)
...
let string_of_kind_geo kind cc =
  ...
  try
    match kind with
    | Known_location (ip, port) -> ip_cn_cc ip
    | Indirect_location (server_ip, server_port, ip, port) -> ip_cn_cc ip
...
type kind_type = { f : string -> string -> unit; description : string }
...
type web_infos_state = DownloadStarted | FileLoaded
