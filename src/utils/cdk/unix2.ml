...
let tryopen_read_gzip fn f = 
  tryopen Gzip.open_in_chan Gzip.close_in fn f
let tryopen_write_gzip ?level fn f = 
  tryopen (Gzip.open_out_chan ?level) Gzip.close_out fn f
...
let can_write_to_directory dirname =
  ...
  let check () = with_remove temp_file (fun _ ->
    tryopen_openfile temp_file [O_WRONLY; O_CREAT] 0o600 (fun fd ->
      ...
      really_write fd (Bytes.unsafe_of_string test_string) 0 (String.length test_string)))
  in
  try
    check ()
  with
