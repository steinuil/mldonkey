...
let read_ecd filename ic =
  ...
  let rec find_ecd pos len =
    ...
    let ofs = strrstr "PK\005\006" (Bytes.unsafe_to_string buf) 0 newlen in
    if ofs < 0 || newlen < 22 || 
       (let comment_len = 
  ...
  let comment = readstring ic comment_len |> Bytes.unsafe_to_string in
...
let read_cd filename ic cd_entries cd_offset cd_bound =
  ...
  try
    ...
    while (LargeFile.pos_in ic < cd_bound) do
      ...
      let name = readstring ic name_len |> Bytes.unsafe_to_string in
      let extra = readstring ic extra_len |> Bytes.unsafe_to_string in
      let comment = readstring ic comment_len |> Bytes.unsafe_to_string in
...
let write_directory_entry oc e =
  ...
  writestring oc (Bytes.unsafe_of_string e.filename);            (* filename *)
  writestring oc (Bytes.unsafe_of_string e.extra);               (* extra info *)
  writestring oc (Bytes.unsafe_of_string e.comment)              (* file comment *)
...
let close_out ofile =
  ...
  writestring oc (Bytes.unsafe_of_string ofile.of_comment);         (* comment *)
...
let add_entry_header ofile extra comment level mtime filename =
  ...
  writestring oc (Bytes.unsafe_of_string filename);              (* filename *)
  writestring oc (Bytes.unsafe_of_string extra);                 (* extra info *)
  { filename = filename;
    extra = extra;
    comment = comment;
...
let add_entry data ofile ?(extra = "") ?(comment = "") 
  ...
  let crc = Zlib.update_crc Int32.zero (Bytes.unsafe_of_string data) 0 (String.length data) in
  let compr_size =
    match level with
      0 ->
        output ofile.of_channel (Bytes.unsafe_of_string data) 0 (String.length data);
        String.length data
    | _ ->
        let in_pos = ref 0 in
        let out_pos = ref 0 in
          ...
          Zlib.compress ~header:false
            (fun buf ->
               let n = min (String.length data - !in_pos)
                           (Bytes.length buf) in
               Bytes.blit_string data !in_pos buf 0 n;
               in_pos := !in_pos + n;
               n)
            (fun buf n ->
