...
*)
  ...
  let purge_map f map =
    ...
    Intmap.iter
      (fun num doc -> if f doc then new_map := Intmap.add num doc !new_map)
      map;
    !new_map

  let merge_maps map1 map2 =
    if map1 = Intmap.empty then map2
    else
      let new_map = ref map1 in
      Intmap.iter
        (fun num doc ->
          if not (Intmap.mem num !new_map) then
            new_map := Intmap.add num doc !new_map)
        map2;
      !new_map

  let substract_map map map_not =
    if map_not = Intmap.empty then map
    else
      let new_map = ref Intmap.empty in
      Intmap.iter
        (fun num doc ->
  ...
  let rec query_map idx q =
    try
      ...
      | And ((Predicate _ as q2), q1) | And (q1, q2) ->
      ...
      | Predicate f -> Intmap.empty
    with _ -> Intmap.empty

  and and_query idx q map =
    try
      if map = Intmap.empty then map
      else
        match q with
        | HasWord s ->
            let tree = find idx s in
  ...
  and andnot_query idx q map =
    ...
    | And ((Predicate _ as q2), q1) | And (q1, q2) ->
        ...
        substract_map map map_not
    | Or (q, Predicate f) | Or (Predicate f, q) ->
    ...
    | AndNot (q1, q2) ->
        ...
        substract_map map map_not
    | Predicate f -> purge_map (fun x -> not (f x)) map
  ...
  let rec simplify q =
    ...
    | Predicate _ | HasField _ | HasWord _ -> q

  and simplify1 q =
    match q with
    | And (Predicate f1, Predicate f2) -> Predicate (fun x -> f1 x && f2 x)
    ...
    | Or (q1, Or (q2, q3)) -> simplify1 (Or (simplify1 (Or (q1, q2)), q3))
    | And (q1, And (q2, q3)) -> simplify1 (And (simplify1 (And (q1, q2)), q3))
    | And (Or (q1, q2), q3) ->
...
module FullMake (Doc : Doc) (Make : Make) = struct
