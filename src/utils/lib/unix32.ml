...
let mini (x : int) (y : int) = if x > y then y else x
...
module FDCache = struct
  ...
  let close t =
    if not t.destroyed then
      ...
      | Some fd ->
          ...
          ( try Unix.close fd
            with e ->
              lprintf_nl "Exception in FDCache.close %s: %s" t.filename
  ...
  let is_closed t =
    if t.destroyed then true
    else match t.fd with Some _ -> false | None -> true
  ...
  let rec close_one () =
    if not (Fifo.empty cache) then
      ...
      match t.fd with
      | None -> close_one ()
      | Some _ -> ( try close t with _ -> close_one () )

  let check_destroyed t =
    if t.destroyed then
      failwith
        (Printf.sprintf "Unix32.check_destroyed %s: Cannot use destroyed FD"
           t.filename)
  ...
  let _local_force_fd creat t =
    ...
    let fd =
      ...
      | None ->
          ...
          let fd =
            Unix2.tryopen_umask 0 (fun _old_umask ->
                ...
                try
                  if t.writable then
                    Unix.openfile t.filename
                      ...
                      !create_file_mode
                  else Unix.openfile t.filename ro_flag 0o400
                with e ->
                  if !verbose then
                    lprintf_nl
                      "Exception in FDCache._local_force_fd %s (%s): %s"
                      t.filename
  ...
  let create f writable =
    if !verbose then
      lprintf_nl "Open %s (%s)" f (if writable then "rw" else "ro");
    let t = { filename = f; writable; fd = None; destroyed = false } in
    try
  ...
  let rename t f =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.rename %s %s: %s" t.filename f
          (Printexc2.to_string e);
      raise e
  ...
  let ftruncate64 t len sparse =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.ftruncate64 %s %Ld (%s): %s" t.filename
          len
  ...
  let getsize64 t =
    ...
    | e ->
        if !verbose then
          lprintf_nl "Exception in FDCache.getsize64 %s: %s" t.filename
            (Printexc2.to_string e);
        raise e
  ...
  let owner t =
    ...
    | e ->
        if !verbose then
          lprintf_nl "Exception in FDCache.owner %s: %s" t.filename
            (Printexc2.to_string e);
        raise e
  ...
  let mtime64 t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.mtime64 %s: %s" t.filename
          (Printexc2.to_string e);
      raise e
  ...
  let exists t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.exists %s: %s" t.filename
          (Printexc2.to_string e);
      raise e
  ...
  let remove t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.remove %s: %s" t.filename
          (Printexc2.to_string e);
      raise e
  ...
  let multi_rename t f file =
    try
      ...
      (* ( try Unix2.rename t.filename (Filename.concat f file)
         with
         | Unix.Unix_error (Unix.EACCES, _, _)
         when Autoconf.windows && Unix2.list_directory t.filename = []
         ->
           remove t ); *)
      Unix2.rename t.filename (Filename.concat f file);
      destroy t
    with e ->
      lprintf_nl "Exception in FDCache.multi_rename %s %s: %s" t.filename
        (Filename.concat f file) (Printexc2.to_string e);
      raise e
  ...
  let read file file_pos string string_pos len =
    try
      ...
      if !verbose then
        lprintf_nl "really_read %s %Ld %d" file.filename file_pos len;
      Unix2.really_read fd string string_pos len
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.read %s %Ld %d: %s" file.filename
          file_pos len (Printexc2.to_string e);
      raise e
  ...
  let write file file_pos string string_pos len =
    try
      ...
      ignore (Unix2.c_seek64 fd file_pos Unix.SEEK_SET);
      if !verbose then
        lprintf_nl "really_write %s %Ld %d" file.filename file_pos len;
      really_write fd string string_pos len
    with e ->
      if !verbose then
        lprintf_nl
          "Exception in FDCache.write file %s file_pos=%Ld len=%d \
           string_pos=%d, string length=%d: %s"
          file.filename file_pos len string_pos (Bytes.length string)
          (Printexc2.to_string e);
      raise e
  ...
  let copy_chunk t1 t2 pos1 pos2 len64 =
    ...
    in
    try iter len64 pos1 pos2
    with e ->
      if !verbose then
        lprintf_nl "Exception in FDCache.copy_chunk %s %Ld to %s %Ld (%Ld): %s"
          t1.filename pos1 t2.filename pos2 len64 (Printexc2.to_string e);
      raise e
end
...
module type File = sig
  ...
  val apply_on_chunk :
    t -> int64 -> int64 -> (Unix.file_descr -> int64 -> 'a) -> 'a
...
module DiskFile = struct
  ...
  let apply_on_chunk t pos_s _len_s f =
...
module MultiFile = struct
  ...
  and btree = Node of int64 * btree * btree | Leaf of file
  ...
  let make_tree files =
    make_tree (Array.of_list (List.filter (fun file -> file.len > zero) files))

  let rec find_file tree file_pos =
    match tree with
    | Leaf file -> file
    | Node (pos, tree1, tree2) ->
        find_file (if file_pos < pos then tree1 else tree2) file_pos

  let rec subfile_tree_map indent tree f =
    match tree with
    | Leaf file -> f file.filename file.pos file.len file.current_len
    | Node (_pos, tree1, tree2) ->
  ...
  let rec print_tree indent tree =
    match tree with
    | Leaf file ->
        lprintf_nl "%s  - %s (%Ld,%Ld)" indent file.filename file.pos file.len
    | Node (pos, tree1, tree2) ->
  ...
  let do_on_remaining tail arg remaining f =
    let rec iter_files list arg remaining =
      ...
      | file :: tail ->
          ...
          if remaining > zero then iter_files tail arg remaining
    in
  ...
  let rec fill_zeros file_out file_pos max_len =
    if max_len > zero then (
      ...
      FDCache.copy_chunk (zero_chunk_fd ()) file_out zero file_pos
        max_possible_write;
      fill_zeros file_out
        (file_pos ++ max_possible_write)
  ...
  let apply_on_chunk t chunk_begin chunk_len f =
    ...
    if max_current_pos >= chunk_end then
      ...
      try
        ...
        FDCache.copy_chunk file.fd file_out in_pos zero in_len;
        let zeros = min (chunk_len -- in_len) (file.len -- file.current_len) in
        ...
        do_on_remaining tail in_len (chunk_len -- in_len)
          (fun file file_pos len ->
            ...
            FDCache.copy_chunk file.fd file_out zero file_pos max_current_pos;
      ...
      with e ->
        if !verbose then
          lprintf_nl "Exception in MultiFile.apply_on_chunk %s %Ld %Ld: %s"
            t.dirname chunk_begin chunk_len (Printexc2.to_string e);
        (try FDCache.close file_out with _ -> ());
        (try Sys.remove temp_file with _ -> ());
        raise e

  let close t = List.iter (fun file -> FDCache.close file.fd) t.files

  let destroy t = List.iter (fun file -> FDCache.destroy file.fd) t.files

  let rename t f =
    close t;
    List.iter (fun file -> FDCache.multi_rename file.fd f file.filename) t.files

  let ftruncate64 t size _sparse = t.size <- size
  ...
  let mtime64 t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in MultiFile.mtime64 %s: %s" t.dirname
          (Printexc2.to_string e);
      raise e

  let exists t = Sys.file_exists t.dirname

  let remove t =
    try
      close t;
      if Sys.file_exists t.dirname then Unix2.remove_all_directory t.dirname
    with e ->
      if !verbose then
        lprintf_nl "Exception in MultiFile.remove %s: %s" t.dirname
          (Printexc2.to_string e);
      raise e
  ...
  let io f t chunk_begin string string_pos len =
    ...
    if file_end >= chunk_end then
      f file (chunk_begin -- file_begin) string string_pos len
    else
      (* first file *)
      let in_pos = chunk_begin -- file_begin in
      ...
      (* other files *)
      do_on_remaining tail (string_pos + in_len) (chunk_len -- first_read)
...
module SparseFile = struct
  ...
  let create filename writable =
    ...
    { filename; dirname; chunks = [||]; size = zero; writable }
  ...
  let find_read_pos t pos =
    ...
    let rec iter t start len =
      if len <= 0 then start
        ...
        else if chunk.pos > pos then
          if med = start then start else iter t start milen
        else
          let next = med + 1 in
  ...
  *)
    ...
    then
      ...
      let rec iter pos index chunk_begin chunk_len =
        if chunk_len > zero then (
          ...
          FDCache.copy_chunk chunk.fd file_out in_chunk_pos pos max_len;
  ...
  let close t = Array.iter (fun file -> FDCache.close file.fd) t.chunks

  let destroy t = Array.iter (fun file -> FDCache.destroy file.fd) t.chunks
  ...
  *)
  ...
  let mtime64 t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in SparseFile.mtime64 %s: %s" t.dirname
          (Printexc2.to_string e);
      raise e
  ...
  let remove t =
    ...
    with e ->
      if !verbose then
        lprintf_nl "Exception in SparseFile.remove %s: %s" t.dirname
          (Printexc2.to_string e);
      raise e
  ...
  let write t chunk_begin string string_pos len =
    ...
    if index = Array.length t.chunks then (
      ...
      let chunk =
        { chunkname = chunk_name; pos = chunk_begin; len = zero; fd }
      in
      let new_array = Array.make (nchunks + 1) chunk in
      Array.blit t.chunks 0 new_array 0 nchunks;
      t.chunks <- new_array )
    else if t.chunks.(index).pos > chunk_begin then (
    ...
    let rec iter index chunk_begin string_pos len =
      if len > 0 then (
        let next_index, max_len, max_len64 =
          if index = nchunks - 1 then (index, len, Int64.of_int len)
          else
        ...
        iter next_index (chunk_begin ++ max_len64) (string_pos + max_len)
          (len - max_len) )
    in
...
let create f writable creator =
  try
    let fd = H.find table { dummy with filename = f; writable } in
...
let find_writable fd =
  if fd.writable then Some fd
    ...
    try Some (H.find table { fd with writable = true }) with Not_found -> None
...
let create_sparsefile filename writable =
  create filename writable (fun f -> SparseFile (SparseFile.create f writable))
...
let write file file_pos string string_pos len =
  ...
  else lprintf_nl "Unix32.write: error, invalid argument len = 0"
...
*)
  ...
  with e ->
    ...
    t.buffers <-
      (Bytes.to_string s, 0, len, offset, Int64.of_int len) :: t.buffers;
    raise e
      ...
      if t.buffers = [] then ()
        ...
        and iter_in offset len =
          ...
          | (s, pos_s, len_s, offset2, len2) :: tail ->
              ...
              if in_offset = Int64.zero then (
                Buffer.add_substring buffer s pos_s len_s;
                t.buffers <- tail;
                iter_in offset (len ++ len2) )
              else if in_offset < Int64.zero then (
                flush_buffer t offset;
                iter_out () )
              else
                ...
                if keep_len <= 0L then (
                  ...
                  Buffer.add_substring buffer s
                    (pos_s + Int64.to_int new_pos)
                    (Int64.to_int keep_len);
...
let read t file_pos string string_pos len =
  ...
  | Destroyed ->
      failwith (Printf.sprintf "Unix32.read on destroyed FD %s" (filename t))
...
let flush _ =
  ...
  with e ->
    if !verbose then
      lprintf_nl "[ERROR] Exception %s in Unix32.flush" (Printexc2.to_string e)
...
let rec copy t1 t2 pos1 pos2 len64 =
  ...
  else copy_chunk t1 t2 pos1 pos2 (Int64.to_int len64)
...
let file_exists s =
  ...
  try
    let fd = create_ro s in
    let exists =
      try exists fd with Unix.Unix_error (Unix.ENOENT, _, _) -> false
    in
    if not old_fd_exists then close fd;
    exists
...
let owner s =
  let old_fd_exists = fd_exists s in
  let fd = create_ro s in
  let user, pass = try owner_fd fd with _ -> ("", "") in
  if not old_fd_exists then close fd;
  (user, pass)
...
module DiskFile_Test : File = DiskFile

module SparseFile_Test : File = SparseFile
...
module SharedParts = struct
  ...
  *)
  ...
  type t = { file : file; mutable file_parts : part list }
  ...
  let copy_shared_parts_in file parts =
    List2.tail_map
      (fun part ->
        if part.part_file != file then (
          lprintf_nl "Copy shared part to another file";
          copy part.part_file file part.part_begin part.part_begin part.part_len;
          lprintf_nl "   Copy done.";
          part.part_shared <-
            List.filter (fun t -> t.file != file) part.part_shared;
          { part with part_file = file; part_shared = [] } )
        else part)
      parts
  ...
  let apply_on_parts f t file_pos s pos len =
    List.iter (fun part -> f part.part_file file_pos s pos len) t.file_parts
  ...
  let write t file_pos s pos len = apply_on_parts write t file_pos s pos len

  let read t file_pos s pos len = apply_on_parts read t file_pos s pos len
  ...
  let apply_on_chunk t chunk_begin chunk_len f =
    ...
    let rec iter list =
      ...
      | [] -> assert false
      | part :: tail ->
          if part.part_begin <= chunk_begin && part.part_end >= chunk_end then
            apply_on_chunk part.part_file chunk_begin chunk_len f
          else if part.part_end > chunk_begin then make_temp_file list
          else iter tail
        ...
        if chunk_len > zero then
          ...
          | part :: tail ->
              ...
              copy_chunk part.part_file file_out chunk_begin pos
                (Int64.to_int tocopy);
              fill (pos ++ tocopy) (chunk_begin ++ tocopy) (chunk_len -- tocopy)
                tail
      in
  ...
  let ftruncate64 t len sparse = ftruncate64 t.file len sparse
  ...
  let create_ro filename = create (create_ro filename)

  let create_rw filename = create (create_rw filename)
...
let subfile_tree_map t f =
  match t.file_kind with
  | MultiFile t ->
      MultiFile.subfile_tree_map "" t.MultiFile.tree f;
      ()
  | _ -> ()
...
let find_file t chunk_begin =
  match t.file_kind with
  | MultiFile t ->
      let sf, _tail = MultiFile.find_file t chunk_begin in
...
type statfs = {
  ...
  f_fsid : unit;
  (* See note in statfs(2) *)
  f_fnamelen : int64;
  (* maximum length of filenames, maximum Sys.max_string_length *)
  f_basetype : string;
  (* type of filesystem - Solaris, (-1) on other systems, use f_type there *)
  f_frsize : int64;
...
let _bsize sf =
  try
    if sf.f_frsize = Int64.zero || sf.f_frsize = -1L then Some sf.f_bsize
    else Some sf.f_frsize
  with _ -> None

let bsize dir = try _bsize (statfs dir) with _ -> None

let blocks dir = try Some (statfs dir).f_blocks with _ -> None

let bfree dir = try Some (statfs dir).f_bfree with _ -> None

let bavail dir = try Some (statfs dir).f_bavail with _ -> None

let fnamelen dir =
  try
    Some
      (Int64.to_int
         (min (statfs dir).f_fnamelen (Int64.of_int Sys.max_string_length)))
  with _ -> None
...
let disktotal dir =
  ...
  try
    let s = statfs dir in
    match _bsize s with
    | Some bsize -> Some (bsize ** s.f_blocks)
    | None -> None
  with _ -> None
...
let diskfree dir =
  ...
  try
    let s = statfs dir in
    match _bsize s with
    | Some bsize -> Some (bsize ** s.f_bavail)
    | None -> None
  with _ -> None
...
let diskused dir =
  ...
  try
    let s = statfs dir in
    match _bsize s with
    | Some bsize -> Some (bsize ** (s.f_blocks -- s.f_bavail))
    | None -> None
  with _ -> None
...
let percentused dir =
  ...
  | Some dfree, Some dtotal -> (
      try Some (Int64.to_int (100L -- ((dfree ** 100L) // dtotal)))
...
let percentfree dir =
  ...
  match (diskfree dir, disktotal dir) with
  | Some dfree, Some dtotal -> (
      try Some (Int64.to_int ((dfree ** 100L) // dtotal))
      with Division_by_zero -> None )
  | _ -> None
...
let filesystem' dir =
  try
    ...
    | 0x4000L ->
        ("ISOFS_SUPER_MAGIC_WIN", `Win) (* from coreutils-5.2.1, stat.c *)
    | 0x4004L ->
        ("ISOFS_SUPER_MAGIC_R_WIN", `Win) (* from coreutils-5.2.1, stat.c *)
    ...
    | _ ->
        if s.f_basetype <> "-1" then (s.f_basetype, `Unknown)
        else (Printf.sprintf "unknown (%LX)" s.f_type, `Unknown)
  with _ -> ("not supported", `Unknown)
...
let set_max_cache_size v =
  max_cache_size := v;
  while !FDCache.cache_size > !max_cache_size do
    FDCache.close_one ()
  done
