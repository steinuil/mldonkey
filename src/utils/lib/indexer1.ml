...
module Make (Doc : Doc) = struct
  ...
  type tree_rec = { mutable tree_expr : node }
  ...
  *)
      ...
      if pos = len then
        ...
        | String (str, num) ->
            if str = "" then (num, filtered)
              ...
              (new_num, filtered)
        | Filtered tree -> iter pos tree true
      else
        match tree.tree_expr with
        | Suffix _ -> assert false
        | String (str, num) ->
            let suffix = String.sub s pos (len - pos) in
            if str = suffix then (num, filtered)
            else if str = "" then (
              let new_num = new_word (String.sub s 0 pos) in
              tree.tree_expr <-
                List
        ...
        | List (num, list) -> (
            let c = s.[pos] in
            try iter (pos + 1) (List.assoc c list) filtered
            with Not_found ->
  ...
  let add_doc w doc field filtered =
    if filtered || w.word_filtered then
      (*      lprintf "FILTER ON DOC"; lprint_newline (); *)
      Doc.filter doc true;
    w.word_docs <-
      ( match w.word_docs with
      | (d, f) :: tail when d = doc -> (doc, f lor field) :: w.word_docs
        ...
        let rec iter pos =
          ...
          if pos < len then
            ...
            | None ->
                ...
                | ww :: _ when w == ww -> ()
                | _ -> suffix.suffix_words <- w :: suffix.suffix_words );
                iter (pos + 1)
        in
        iter 1
  ...
  let find idx s =
    try
      ...
      let rec iter pos tree =
        if pos = len then tree.tree_expr
        else
          match tree.tree_expr with
          | Suffix _ -> assert false
          | String (str, num) ->
              ...
              if str1 = str2 then Suffix num else raise Not_found
          | Filtered _ -> raise Not_found
          | List (num, list) ->
              if pos = len then Suffix num
              else
                let c = s.[pos] in
  ...
  let print idx =
    ...
    let rec iter pos tree =
      ...
      | String (s, num) ->
          ...
          spaces pos;
          lprintf "  ";
          List.iter (fun w -> lprintf_nl "%s " w.word_string) num.suffix_words
      | List (num, list) ->
          spaces pos;
  ...
  let print_word_docs w =
    ...
    List.iter (fun (doc, _) -> lprintf_nl "    %d" (Doc.num doc)) w.word_docs

  let print_suffix_docs suffix =
    List.iter
      (fun w -> print_word_docs w)
      ( match suffix.suffix_word with
  ...
  let rec filter_tree tree =
    ...
    | List (s, list) ->
        ...
        filter_suffix s
    | String (_, s) | Suffix s -> filter_suffix s
    | Filtered t -> filter_tree t
    ...
    List.iter
      (fun w ->
        if not w.word_filtered then (
          ...
          List.iter
            (fun (doc, where) ->
              if not (Doc.filtered doc) then Doc.filter doc true)
            w.word_docs ))
      ( match s.suffix_word with
      | None -> s.suffix_words
      | Some w -> w :: s.suffix_words )
    ...
    let rec iter pos tree =
      if pos = len then (
        ...
        | List (num, list) -> (
            ...
            let c = s.[pos] in
            try iter (pos + 1) (List.assoc c list)
            with Not_found ->
              let new_num = new_word s in
              let new_tree =
                {
  ...
  let clear_filter index =
    let rec iter tree =
      ...
      | List (s, list) ->
          ...
          iter_suffix s
      | String (_, s) | Suffix s -> iter_suffix s
      | Filtered t ->
  ...
  *)
  ...
  let or_get_fields map tree fields =
    ...
    let rec iter tree =
      ...
      | List (s, tree_list) ->
          iter_suffix s;
          List.iter (fun (_, tree) -> iter tree.tree_expr) tree_list
      | String (_, s) | Suffix s -> iter_suffix s
      | Filtered _ -> ()
    and iter_suffix s =
            ...
            List.iter
              (fun (doc, where) ->
                ...
                if
                  ...
                  && not (Intmap.mem (Doc.num doc) !map)
                then
