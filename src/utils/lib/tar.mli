...
type file_type =
  ...
  | LONGLINK
      (** GNU: Identifies the *next* file on the archive as having a long linkname. The body of this entry is the linkname. *)
  | LONGNAME
      (** GNU: Identifies the *next* file on the tape as having a long name. The body of this entry is the filename. *)
  | MULTIVOL
      (** GNU: The continuation of a file that began on another volume. *)
  | NAMES
      (** GNU: For storing filenames that do not fit into the main header. I haven't yet figured out /where/ they're stored. *)
...
type header = {
  ...
  t_gnu : gnu_extras option;
      (** These aren't always set to meaningful values even with [OLDGNU_FORMAT] and [GNU_FORMAT] records. *)
}
...
val open_in :
  ?compress:[< `Plain | `Gzip | `Bzip2 | `Guess > `Guess ] -> string -> t_in
...
val open_in_chan :
  ?compress:[< `Plain | `Gzip | `Bzip2 > `Plain ] -> in_channel -> t_in
...
val open_out :
  ?compress:[< `Plain | `Gzip | `Bzip2 > `Plain ] -> string -> t_out
(** Create a new tar file, optionally compressed with gzip or
  bzip2. *)

val open_out_chan :
  ?compress:[< `Plain | `Gzip | `Bzip2 > `Plain ] -> out_channel -> t_out
