...
type option_info = {
  ...
  option_public : bool;
  (* send this option to GUIs even for non-admin users *)
...
val hasharray_option :
  'a -> (int * 'a * 'b) option_class -> ('a, 'b) Hashtbl.t array option_class
...
val set_string_wrappers :
  'a option_class -> ('a -> string) -> (string -> 'a) -> unit

val simple_args : string -> options_file -> (string * Arg.spec * string) list

val prefixed_args : string -> options_file -> (string * Arg.spec * string) list
...
val strings_of_section_options : string -> options_section -> option_info list
