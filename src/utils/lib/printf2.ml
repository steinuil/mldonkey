...
let lprintf_handler =
  ref (fun s time -> Printf.printf "%sMessage [%s] discarded\n" time s)

let show_exn exn s =
  match exn with
  | None -> s ^ "\n"
  | Some exn ->
      Printf.sprintf "Exception in %s : %s\n" s (Printexc2.to_string exn)

let lprintf fmt =
  Printf.ksprintf (fun s -> try !lprintf_handler "" s with _ -> ()) fmt

let lprintf2 m fmt =
  Printf.ksprintf
    (fun s -> try !lprintf_handler (log_time ()) (m ^ " " ^ s) with _ -> ())
    fmt

let lprintf_nl ?exn fmt =
  Printf.ksprintf
    (fun s ->
      try !lprintf_handler (log_time ()) (show_exn exn s) with _ -> ())
    fmt

let lprintf_nl2 ?exn m fmt =
  Printf.ksprintf
    (fun s ->
      try !lprintf_handler (log_time ()) (m ^ " " ^ show_exn exn s)
      with _ -> ())
    fmt
...
let set_lprintf_handler f = lprintf_handler := f
...
let close_log () =
  ...
  | None -> ()
  | Some oc ->
      if oc != stderr && oc != stdout then close_out oc;
      lprintf_output := None
...
let html_mods_commands buf n c l =
  (* Name Class List *)
  Printf.bprintf buf
    "\\<div class=\\\"%s\\\"\\>\\<table id=\\\"%s\\\" name=\\\"%s\\\" \
     class=\\\"%s\\\" cellspacing=0 cellpadding=0 \
    ...
    (fun (w, x, y, z) ->
      ...
      Printf.bprintf buf
        "\\<td class=\\\"%s\\\" title=\\\"%s\\\" \
         ...
         onMouseOut=\\\"mOut(this);\\\" onClick=\\\"%s\\\" \\>%s\\</td\\>"
        w x y z)
    l;
...
type sort_kind =
  | Num
  (* numeric, parse size suffixes (kMGT) *)
  | Str
  ...
  if List.length l > 0 then (
    ...
    List.iter
      (fun (w, x, y, z) ->
        ...
        Printf.bprintf buf
          "\\<td onClick=\\\"_tabSort(this,%s,%s);\\\" class=\\\"%s\\\" \
           title=\\\"%s\\\"\\>%s\\</td\\>"
          sort total x y z)
      l;
    Printf.bprintf buf "\\</tr\\>" )
...
let html_mods_table_header_colspan buf ?(total = "0") n c l =
  ...
  Printf.bprintf buf
    "\\<div class=\\\"%s\\\"\\>\\<table id=\\\"%s\\\" name=\\\"%s\\\" \
     class=\\\"%s\\\" cellspacing=0 cellpadding=0\\>\\<tr\\>"
    c n n c;
  List.iter
    (fun (v, w, x, y, z) ->
      (* Sort Class Title Value *)
      Printf.bprintf buf
        "\\<td colspan=%s onClick=\\\"_tabSort(this,%s,%s);\\\" \
...
let html_mods_table_no_header buf n c l =
  ...
  List.iter
    (fun (t, c, d) ->
      (* Title Class Value *)
      Printf.bprintf buf "\\<td class=\\\"%s\\\" %s\\>%s\\</td\\>" c
        (if t <> "" then "title=\\\"" ^ t ^ "\\\"" else "")
        d)
    l;
...
let html_mods_big_header_start buf c l =
  Printf.bprintf buf
    "\\<div class=\\\"%s\\\"\\>\\<table class=main border=0 cellspacing=0 \
     cellpadding=0\\>\\<tr\\>\\<td\\>"
    c;
  Printf.bprintf buf
  ...
  List.iter
    (fun s ->
      incr cnt;
      Printf.bprintf buf "\\<td nowrap class=\\\"fbig%s\\\"\\>%s\\</td\\>"
        (if !cnt = len then " pr" else "")
        s)
    l;
...
let html_mods_cntr_init () = html_mods_counter := true

let print_plural_s v = if v > 1 then "s" else ""
