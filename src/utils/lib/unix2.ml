...
let with_remove fn f =
  tryopen (fun fn -> fn) (fun fn -> exn_drop Sys.remove fn) fn f
...
let tryopen_read_tar fn f = tryopen Tar.open_in Tar.close_in fn f

let tryopen_write_tar ?compress fn f =
  tryopen (Tar.open_out ?compress) Tar.close_out fn f

let tryopen_read_gzip fn f = tryopen Gzip.open_in Gzip.close_in fn f

let tryopen_write_gzip ?level:_ fn f = tryopen Gzip.open_out Gzip.close_out fn f
      ...
      try
        while true do
          ...
          if
            ...
            && not
                 ( file = ".DS_Store"
                 || String.sub file 0 2 = "._"
                 || file = "Thumbs.db" || file = "desktop.ini" )
          then list := file :: !list
        done
      with End_of_file -> ());
  !list
...
let iter_directory f dirname =
  tryopen_dir dirname (fun dir ->
      try
        while true do
          let file = readdir dir in
          if file <> "." && file <> ".." then f (Filename.concat dirname file)
        done
      with End_of_file -> ())

let is_directory filename =
  try
    let s = Unix.LargeFile.stat filename in
    s.LargeFile.st_kind = S_DIR
  with _ -> false

let is_link filename =
  try
    let s = Unix.LargeFile.lstat filename in
    s.LargeFile.st_kind = S_LNK
  with _ -> false
...
let rec safe_mkdir ?(mode = 0o775) dir =
  ...
  else if is_link dir then (
    try tryopen_dir dir ignore with
...
let rec really_write fd s pos len =
  if len = 0 then
    ...
    if nwrite = 0 then raise End_of_file
    else if nwrite < len then really_write fd s (pos + nwrite) (len - nwrite)

let rec really_read fd s pos len =
  let nread = Unix.read fd s pos len in
  if nread = 0 then raise End_of_file
  else if nread < len then really_read fd s (pos + nread) (len - nread)

let copy oldname newname =
  tryopen_read_bin oldname (fun ic ->
      let stats =
        try Some (Unix.LargeFile.fstat (Unix.descr_of_in_channel ic))
        with _ -> None
      in
      tryopen_write_bin newname (fun oc ->
          ...
          | Some stats -> (
              ...
              ( try
                  Unix.fchown descr stats.Unix.LargeFile.st_uid
                    stats.Unix.LargeFile.st_gid
                with e ->
                  lprintf_nl "copy: failed to preserve owner : %s"
                    (Printexc.to_string e) );
              try Unix.fchmod descr stats.Unix.LargeFile.st_perm
              with e ->
                lprintf_nl "copy: failed to preserve mode : %s"
                  (Printexc.to_string e) ) );
...
let rename oldname newname =
  if oldname <> newname then
    try Unix.rename oldname newname
    with Unix_error (EXDEV, _, _) -> (
...
let rec remove_all_directory dirname =
  ...
  List.iter
    (fun file ->
      ...
      if is_directory filename then remove_all_directory filename
...
let can_write_to_directory dirname =
  ...
  let check () =
    with_remove temp_file (fun _ ->
        tryopen_openfile temp_file [ O_WRONLY; O_CREAT ] 0o600 (fun fd ->
            let test_string =
              "mldonkey accesstest - this file can be deleted\n"
            in
            really_write fd
  ...
  | Unix.Unix_error (error, func, what) ->
      lprintf_nl "%s(%s) : %s for directory %s" func what (error_message error)
        dirname;
      exit 73
  | e ->
...
type rlimit = { rlim_cur : int; rlim_max : int }

let dummy_rlimit = { rlim_cur = -1; rlim_max = -1 }

external getrlimit : rlimit_resource -> rlimit = "ml_getrlimit"

external setrlimit : rlimit_resource -> rlimit -> unit = "ml_setrlimit"

let ml_getrlimit resource = try getrlimit resource with _ -> dummy_rlimit
