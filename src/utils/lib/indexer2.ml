...
struct
  ...
  type filtered_node = None | Some of node | Filtered of node
  ...
  type index = { mutable node : node }

  let stats index =
    let mem = ref 2 in
    let rec iter node =
      mem :=
        !mem + 9 + Array.length node.docs + Array.length node.fields
  ...
  let new_node () =
    { next_doc = 0; docs = [||]; fields = [||]; nodes = [||]; ndocs = 0 }
  ...
  let add index string doc fields =
    ...
    try
      ...
      let rec iter pos node =
        ...
        if pos = len then
          if add_doc node doc fields then (
            node.ndocs <- node.ndocs + 1;
            true )
          else false
        else
  ...
  let find node s =
    ...
    in
    try iter node.node 0 with _ -> empty
    ...
    let rec iter node =
      for i = 0 to node.next_doc - 1 do
        if node.fields.(i) land fields <> 0 then
          ...
          if (not (Doc.filtered doc)) && not (Intmap.mem (Doc.num doc) !map)
          then map := Intmap.add (Doc.num doc) doc !map
      done;
