...
let bal l x r =
  ...
  if hl > hr + 2 then
    ...
    | Node (ll, lv, lr, _) -> (
        if height ll >= height lr then create ll lv (create lr x r)
        else
          match lr with
          | Empty -> invalid_arg "Set.bal"
          | Node (lrl, lrv, lrr, _) ->
    ...
    | Empty -> invalid_arg "Set.bal"
    | Node (rl, rv, rr, _) -> (
        if height rr >= height rl then create (create l x rl) rv rr
        else
          match rl with
          | Empty -> invalid_arg "Set.bal"
          | Node (rll, rlv, rlr, _) ->
...
let rec split x = function
  ...
  | Node (l, v, r, _) ->
      ...
      if c = 0 then (l, Some v, r)
        ...
        let ll, vl, rl = split x l in
        (ll, vl, join rl v r)
      else
        let lr, vr, rr = split x r in
        (join l v lr, vr, rr)
...
let rec inter s1 s2 =
  ...
  | Node (l1, v1, r1, _), t2 -> (
      ...
      | l2, Some _, r2 -> join (inter l1 l2) v1 (inter r1 r2) )
