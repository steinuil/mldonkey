...
let rec octal x = if x < 8 then x else (x mod 10) + (8 * octal (x / 10))

let int_of_octal_string s =
  let l = String.length s in
  let octal_of_char c = int_of_char c - int_of_char '0' in
  let rec octal_aux acc i =
    if i < l then octal_aux ((acc * 8) + octal_of_char s.[i]) (i + 1) else acc
  in
  octal_aux 0 0
...
let percentage_of_ints v percent =
  int_of_float ((float_of_int v *. float_of_int percent /. 100.0) +. 0.5)

let zip_extract_entry ifile e =
  if e.Zip.is_directory then
    try Unix.mkdir e.Zip.filename 0o777
    with Unix.Unix_error (Unix.EEXIST, _, _) -> ()
  else Zip.copy_entry_to_file ifile e e.Zip.filename
...
let rec zip_add_entry oc file =
  ...
  | Unix.S_REG -> Zip.copy_file_to_entry file oc ~mtime:s.U.st_mtime file
  | Unix.S_DIR ->
