...
external convert_string : string -> string -> string -> string
  = "ml_convert_string"
...
let utf8_look s i =
  ...
  in
  if n' lsr 31 = 0 then n' else invalid_arg "utf8_look char_of_uint"
...
let rec nth_aux s i n = if n = 0 then i else nth_aux s (utf8_next s i) (n - 1)
...
let normalize_language s =
  ...
  if String.length s > 1 then
    if
      ...
      String.sub s 0 2 = "ZH" && String.length s > 4
    then String.sub s 0 5
    else String.sub s 0 2
  else "EN"
...
let georgian = [ [ GEORGIAN_ACADEMY ]; [ GEORGIAN_PS ] ]
...
let romanian =
  [ [ ISO_8859_16; ISO_IR_226; ISO8859_16; ISO_8859_16_2000 ]; [ MACROMANIA ] ]
...
let safe_convert enc s =
  ...
  | "" -> s
  | enc -> (
      try convert ~from_charset:(charset_from_string enc) ~to_charset:UTF_8 s
      with _ -> s )
...
module Locale = struct
  ...
  let () =
    ...
    MlUnix.set_signal Sys.sigint (Sys.Signal_handle (fun _ -> ()));
    MlUnix.set_signal Sys.sigterm (Sys.Signal_handle (fun _ -> ()))
...
*)
  ...
  let charset_list_from_language lang =
    ...
    let _ =
      ...
      | "TR" -> li := turkish :: !li
      | "VI" -> li := vietnamese :: !li
      | "AF" | "AN" | "BR" | "CA" | "DA" | "DE" | "EN" | "ES" | "ET" | "EU"
      | "FI" | "FO" | "FR" | "GA" | "GL" | "GV" | "ID" | "IS" | "IT" | "KL"
      | "KW" | "MS" | "NL" | "NN" | "NO" | "OC" | "PT" | "SQ" | "SV" | "TL"
      | "UZ" | "WA" ->
          li := western_european :: !li
      | _ -> ()
    in
    List.flatten !li
  ...
  let slow_encode_from_utf8 s to_codeset =
    ...
    for i = 0 to slen - 1 do
      try
        ...
        us := !us ^ s'
      with _ -> us := !us ^ char_const
    done;
    !us

  let slow_encode s to_codeset =
    if is_utf8 s then slow_encode_from_utf8 s to_codeset
    else
      let us = ref "" in
  ...
  let fast_encode s to_codeset =
    let rec iter i n =
      if i = n then slow_encode s to_codeset
      else
        try
  ...
  let to_utf8 s =
    if s = "" then s else if is_utf8 s then s else fast_encode s "UTF-8"

  let to_locale s =
    if s = "" || not !conversion_enabled then s
    else
      let s = to_utf8 s in
      match locale with
      | UTF_8 -> s
      | _ -> (
          try convert_string s locale_string "UTF-8"
          with _ -> slow_encode_from_utf8 s locale_string )

  let () = set_default_charset_list default_language
end
