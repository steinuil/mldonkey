...
module Base16 = struct
  ...
  let hexa_digit_case upper x =
    if x >= 10 then Char.chr (Char.code (if upper then 'A' else 'a') + x - 10)
    else Char.chr (Char.code '0' + x)
  ...
  let digit_hexa c =
    ...
    if i >= i_a && i <= i_f then i - i_a + 10
    else if i >= i_A && i <= i_F then i - i_A + 10
    else if i >= i_0 && i <= i_9 then i - i_0
    else failwith "Bad hexa char"
    ...
    for i = 0 to len - 1 do
      ...
      if bit < 3 then
        ...
        if byte + 1 < hash_length then
          ...
          Bytes.set s (byte + 1)
          @@ char_of_int (int_of_char (Bytes.get s (byte + 1)) lor y)
    done;
    s |> Bytes.to_string
  ...
  let to_string hash_length s =
    ...
    for i = 0 to len - 1 do
      ...
      if bit < 3 then
        ...
        let x = int_of_char s.[byte] in
        let y =
          if byte + 1 = hash_length then 0 else int_of_char s.[byte + 1]
        in
        let x = (x lsl 8) + y in
  ...
  let to_string _ hashbin =
    ...
    for i = 0 to 6 do
      let tmp =
        if i < 6 then
          (hashbin (3 * i) lsl 16)
          lor (hashbin ((3 * i) + 1) lsl 8)
          lor hashbin ((3 * i) + 2)
        else (hashbin (3 * i) lsl 16) lor (hashbin ((3 * i) + 1) lsl 8)
      in
      for k = 0 to 3 do
    ...
    Bytes.sub hash64 0 !j |> Bytes.to_string
...
module Make (M : sig
  ...
  val unsafe_string : bytes -> bytes -> int -> unit
...
struct
  ...
  let string s =
    ...
    unsafe_string digest (Bytes.of_string s) len;
  ...
  let digest_subfile fd pos len =
    let digest = Bytes.create hash_length in
    Unix32.apply_on_chunk fd pos len (fun fd pos ->
        digest_subfile digest fd pos len);
    digest |> Bytes.to_string

  let create () = Bytes.create hash_length |> Bytes.to_string
  ...
  let random () =
    let s = create () |> Bytes.of_string in
    for i = 0 to hash_length - 1 do
  ...
  let option = define_option_class hash_name value_to_hash hash_to_value
...
module Md4 = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit = "md4_unsafe_string"
...
module Md5 = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit = "md5_unsafe_string"
...
module PreSha1 = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit = "sha1_unsafe_string"
...
module Sha1 = struct
  ...
  (* try
       ...
       assert (to_string (string (Bytes.of_string s1)) = "3I42H3S6NNFQ2MSVX7XZKYAYSCX5QBYJ");
       assert (to_string (string s2) = "LOUTZHNQZ74T6UVVEHLUEDSD63W2E6CP");
       assert (to_string (string s3) = "ORWD6TJINRJR4BS6RL3W4CWAQ2EDDRVU");
       assert (to_string (string s4) = "UUHHSQPHQXN5X6EMYK6CD7IJ7BHZTE77");
...
module Tiger = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit = "tiger_unsafe_string"

  let unsafe_file _digest _filename =
    failwith "Tiger.unsafe_file not implemented"

  let digest_subfile _ _ _ _ = failwith "Tiger.digest_subfile not implemented"
...
module PreTigerTree = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit
    = "tigertree_unsafe_string"
  ...
  let unsafe_file digest filename file_size =
    ...
    Unix32.apply_on_chunk fd Int64.zero file_size (fun fd pos ->
...
module TigerTree = struct
  ...
  (* try
       assert (to_string (string s1) = "LWPNACQDBZRYXW3VHJVCJ64QBZNGHOHHHZWCLNQ");
       assert (to_string (string s2) = "VK54ZIEEVTWNAUI5D5RDFIL37LX2IQNSTAXFKSA");
       assert (to_string (string s3) = "L66Q4YVNAFWVS23X2HJIRA5ZJ7WXR3F26RSASFA");
       assert (to_string (string s4) = "PZMRYHGY6LTBEH63ZWAHDORHSYTLO4LEFUIKHWY");
       true
     with e ->
...
module PreMd5Ext = Make (struct
  ...
  external unsafe_string : bytes -> bytes -> int -> unit = "fst_hash_string_ml"

  external unsafe_file : bytes -> string -> int64 -> unit = "fst_hash_file_ml"

  let digest_subfile _ _ _ _ = failwith "Md5Ext.digest_subfile not implemented"
