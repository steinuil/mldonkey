...
let writestring oc s = output oc s 0 (String.length s)
...
let read_ecd filename ic =
  ...
  let rec find_ecd pos len =
    ...
    if
      ...
      ||
      let comment_len =
        Char.code buf.[ofs + 20] lor (Char.code buf.[ofs + 21] lsl 8)
      in
      newpos + ofs + 22 + comment_len <> filelen
    ...
    | Deflated ->
        ...
        ( try
            Zlib.uncompress ~header:false
              (fun buf ->
                let read =
                  input ifile.if_channel (Bytes.of_string buf) 0
                    (min !in_avail (String.length buf))
                in
        ...
        let crc =
          Zlib.update_crc Int32.zero (Bytes.to_string res) 0 (Bytes.length res)
        in
        if crc <> e.crc then
...
let copy_entry_to_file ifile e outfilename =
  ...
  try
    ...
    close_out oc;
    try Unix.utimes outfilename e.mtime e.mtime
...
let write_directory_entry oc e =
  ...
  write2 oc 8;
  (* flags *)
  write2 oc (match e.methd with Stored -> 0 | Deflated -> 8);
  (* method *)
  ...
  write4_int oc 0;
  (* external attributes *)
  write4 oc (Int64.to_int32 e.file_offset);
  (* offset of local header *)
  writestring oc e.filename;
  (* filename *)
  writestring oc e.extra;
  (* extra info *)
  writestring oc e.comment
...
let add_entry_header ofile extra comment level mtime filename =
  ...
  write2 oc 8;
  (* flags *)
  write2 oc (if level = 0 then 0 else 8);
  (* method *)
...
let add_entry data ofile ?(extra = "") ?(comment = "") ?(level = 6)
    ...
    | _ -> (
        ...
        try
          Zlib.compress ~header:false
            (fun buf ->
              let n = min (String.length data - !in_pos) (String.length buf) in
...
let copy_channel_to_entry ic ofile ?(extra = "") ?(comment = "") ?(level = 6)
    ?(mtime = Unix.time ()) name =
  let e = add_entry_header ofile extra comment level mtime name in
  let crc = ref Int32.zero in
