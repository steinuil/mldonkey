...
module Make (Ord : OrderedType) = struct
  ...
  type t = { tree : btree; card : int Lazy.t }
  ...
  let rec cardinal_tree = function
    | Empty -> 0
    | Node (l, _, r, _) -> cardinal_tree l + 1 + cardinal_tree r

  let t_of_tree t = { tree = t; card = lazy (cardinal_tree t) }

  let cardinal t = Lazy.force t.card
  ...
  let rec add_tree x = function
    ...
    | Node (l, v, r, _) as t ->
        ...
        if c = 0 then t
        else if c < 0 then bal (add_tree x l) v r
        else bal l v (add_tree x r)
  ...
  let add x t =
    let tree = add_tree x t.tree in
    {
      tree;
      card =
        ( if Lazy.is_val t.card then Lazy.from_val (Lazy.force_val t.card + 1)
        else lazy (cardinal_tree tree) );
    }
  ...
  let rec min_elt_tree = function
    ...
    | Node (Empty, v, _, _) -> v
    | Node (l, _, _, _) -> min_elt_tree l

  let min_elt t = min_elt_tree t.tree

  let rec max_elt_tree = function
    | Empty -> raise Not_found
    | Node (_, v, Empty, _) -> v
    | Node (_, _, r, _) -> max_elt_tree r

  let max_elt t = max_elt_tree t.tree
  ...
  let rec split_tree x = function
    ...
    | Node (l, v, r, _) ->
        ...
        if c = 0 then (l, true, r)
          ...
          let ll, pres, rl = split_tree x l in
          (ll, pres, join rl v r)
        else
          let lr, pres, rr = split_tree x r in
          (join l v lr, pres, rr)
  ...
  let singleton x = { tree = Node (Empty, x, Empty, 1); card = Lazy.from_val 1 }

  let rec remove_tree x = function
    | Empty -> (Empty, false)
    | Node (l, v, r, _) ->
        let c = Ord.compare x v in
        if c = 0 then (merge l r, true)
        else if c < 0 then
  ...
  let rec inter_tree s1 s2 =
    ...
    | Node (l1, v1, r1, _), t2 -> (
        ...
        | l2, false, r2 -> concat (inter_tree l1 l2) (inter_tree r1 r2)
        | l2, true, r2 -> join (inter_tree l1 l2) v1 (inter_tree r1 r2) )

  let inter s1 s2 = t_of_tree (inter_tree s1.tree s2.tree)
  ...
  let rec diff_tree s1 s2 =
    ...
    | Node (l1, v1, r1, _), t2 -> (
        match split_tree v1 t2 with
        | l2, false, r2 -> join (diff_tree l1 l2) v1 (diff_tree r1 r2)
        | l2, true, r2 -> concat (diff_tree l1 l2) (diff_tree r1 r2) )

  let diff s1 s2 = t_of_tree (diff_tree s1.tree s2.tree)
  ...
  let compare_tree s1 s2 = compare_aux (cons_enum s1 End) (cons_enum s2 End)

  let compare s1 s2 = compare_tree s1.tree s2.tree
  ...
  let rec subset_tree s1 s2 =
    ...
    | Node (l1, v1, r1, _), (Node (l2, v2, r2, _) as t2) ->
        let c = Ord.compare v1 v2 in
        if c = 0 then subset_tree l1 l2 && subset_tree r1 r2
        else if c < 0 then
          subset_tree (Node (l1, v1, Empty, 0)) l2 && subset_tree r1 t2
        else subset_tree (Node (Empty, v1, r1, 0)) r2 && subset_tree l1 t2

  let subset s1 s2 = subset_tree s1.tree s2.tree
  ...
  let fold f s accu = fold_tree f s.tree accu

  let rec for_all_tree p = function
    | Empty -> true
    | Node (l, v, r, _) -> p v && for_all_tree p l && for_all_tree p r

  let for_all p t = for_all_tree p t.tree

  let rec exists_tree p = function
    | Empty -> false
    | Node (l, v, r, _) -> p v || exists_tree p l || exists_tree p r

  let exists p t = exists_tree p t.tree

  let filter_tree p s =
    let rec filt accu = function
      | Empty -> accu
      | Node (l, v, r, _) ->
          filt (filt (if p v then add_tree v accu else accu) l) r
    in
    filt Empty s

  let filter p s = t_of_tree (filter_tree p s.tree)

  let partition_tree p s =
    let rec part ((t, f) as accu) = function
      | Empty -> accu
      | Node (l, v, r, _) ->
          part (part (if p v then (add_tree v t, f) else (t, add_tree v f)) l) r
    in
    part (Empty, Empty) s
  ...
  let elements s = elements_aux [] s.tree
