...
type file_type =
  ...
  | CONTIGIOUS
  | DUMPDIR
      (** GNU: A dir entry that contains the names of files that were in the dir at the time the dump was made. *)
  | LONGLINK
      (** GNU: Identifies the *next* file on the archive as having a long linkname *)
  | LONGNAME
      (** GNU: Identifies the *next* file on the tape as having a long name. *)
  | MULTIVOL
      (** GNU: The continuation of a file that began on another volume. *)
  | NAMES
      (** GNU: For storing filenames tha do not fit into the main header. *)
  | SPARSE  (** GNU: Sparse file *)
  | VOLHDR
      (** GNU: This file is a tape/volume header. Ignore it on extraction. *)
...
class in_chan i =
  object
    method really_input str pos len =
      try Stdlib.really_input i str pos len
      with End_of_file -> raise (Error "Unexpected end of file")

    method input str pos len =
      try Stdlib.input i str pos len
      with End_of_file -> raise (Error "Unexpected end of file")
...
class gzin_chan i =
  object
    method really_input str pos len =
      try Gzip.really_input i str pos len
      with End_of_file -> raise (Error "Unexpected end of file")

    method input str pos len =
      try Gzip.input i str pos len
      with End_of_file -> raise (Error "Unexpected end of file")
...
let open_inchan comp chan =
  ...
  | `Bzip2 -> failwith "bzip2 not supported"
...
let pick_comp_type filename = function
  ...
  | `Plain -> `Plain
  | `Guess ->
      if Filename.check_suffix filename ".tar" then `Plain
      else if
        Filename.check_suffix filename ".gz"
        || Filename.check_suffix filename ".Z"
        || Filename.check_suffix filename ".tgz"
      then `Gzip
      else `Plain
...
let open_in_chan ?(compress = `Plain) chan =
  { chan = open_inchan compress chan; rawchan = chan; last_header = None }

let open_in ?(compress = `Guess) filename =
  open_in_chan
    ~compress:(pick_comp_type filename compress)
...
let trim_spaces str pos len =
  let start = ref pos and stop = ref (pos + len - 1) in
  while str.[!start] = ' ' do
    incr start
  done;
  while str.[!stop] = ' ' || str.[!stop] = '\000' do
    decr stop
  done;
...
let extract_num raw pos len =
  if raw.[pos] = '\000' then 0
    ...
    try int_of_string ("0o" ^ trim_spaces raw pos len)
    with Failure _ -> raise (Error "Invalid number in header")

let extract_int32 raw pos len =
  if raw.[pos] = '\000' then 0l
  else
    try Int32.of_string ("0o" ^ trim_spaces raw pos len)
    with Failure _ -> raise (Error "Invalid number in header")
...
let compute_chksum buf =
  let chksum = ref 256 in
  (* 256 is the sum of 8 ' ' characters for the chksum field *)
  for i = 0 to 147 do
...
let read_magic header typec =
  ...
  | "ustar\00000" -> (
      match typec with '0' .. '7' -> POSIX_FORMAT | _ -> GNU_FORMAT )
  | _ -> V7_FORMAT
...
let read_header t =
  ...
  let buf = Bytes.create blocksize in
  t.chan#really_input buf 0 blocksize;
  if buf = Bytes.of_string empty_block then raise End_of_file;
  let buf = Bytes.to_string buf in
  let head1 =
    {
      t_name = c_string buf 0;
  ...
  if chksum <> head1.t_chksum then
    raise
      (Error
         (Printf.sprintf
            "Invalid checksum in tar header. Calculated %d, expected %d" chksum
            head1.t_chksum));
  let head =
...
let align_at_body t =
  match t.last_header with Some _ -> () | None -> ignore (read_header t)
...
class gzout_chan o =
  object
    ...
    method close () : unit = Gzip.close_out o
  end
...
type t_out = { ochan : out_chan; rawochan : out_channel }

let open_out_chan ?(compress = `Plain) chan =
  { ochan = open_outchan compress chan; rawochan = chan }
...
let write_oldgnu_header t buf =
  let ext =
    match t.t_gnu with
    | Some e -> e
    | None -> raise (Error "OLDGNU_FORMAT record without t_gnu field set")
  in
...
let write_gnu_header t buf =
  let ext =
    match t.t_gnu with
    | Some e -> e
    | None -> raise (Error "GNU_FORMAT record without t_gnu field set")
  in
...
let output t head body =
  ...
  if head.t_format = OLDGNU_FORMAT then write_oldgnu_header head buf;
  let chksum = compute_chksum (Bytes.to_string buf) in
  write_padded_num buf 148 chksum;
  t.ochan#output buf 0 blocksize;
  if head.t_format = GNU_FORMAT && isdigit (Bytes.get buf 156) then (
...
let flush t =
  t.ochan#output (Bytes.of_string empty_block) 0 blocksize;
  t.ochan#flush ()
