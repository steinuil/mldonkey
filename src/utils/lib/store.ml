...
let rec iter_write fd s pos len =
  ...
  if nwrite < len then iter_write fd s (pos + nwrite) (len - nwrite)

let really_write fd pos s =
  let len = String.length s in
  if verbose then lprintf_nl "write %d %d" pos len;
...
let rec iter_read fd s pos len =
  ...
  if nread < len then iter_read fd s (pos + nread) (len - nread)

let really_read fd pos s len =
  if verbose then lprintf_nl "read %d %d" pos len;
  ignore (Unix2.c_seek64 fd (Int64.of_int pos) Unix.SEEK_SET);
  iter_read fd s 0 len

(********************* FILE FUNCTIONS *****************)

let gen_file t n = Printf.sprintf "%s_%d" t.store_name n
...
let file_store file str =
  ...
  if pos >= len_all_pos then (
    let new_size = (len_all_pos + 10) * 2 in
    Unix2.c_ftruncate64 file.file_fd
      (Int64.of_int (new_size * file.file_entry_size))
      false;
...
let file_retrieve file pos =
  really_read file.file_fd
    (pos * file.file_entry_size)
    file.file_chunk file.file_entry_size;
  file.file_chunk
...
let rec chunk_size n = if n < 128 then 0 else 1 + chunk_size (n / 2)
...
let save t doc v attr =
  ...
  let file =
    try List.assoc chunk_size t.store_files
    with Not_found ->
  ...
  if verbose then
    lprintf_nl "REALLY WRITE TO %d POS %d LEN %d" chunk_size pos len;
...
let add t v =
  ...
  if doc >= len_all_doc then (
    let new_size = (len_all_doc + 10) * 2 in
    let new_tab = Array.make new_size 0 in
    ( try Array.blit t.store_all_doc 0 new_tab 0 doc
      with e ->
        lprintf_nl "Error in blit %d/%d" doc len_all_doc;
        raise e );
    for i = doc to new_size - 1 do
      new_tab.(i) <- i + 1
    done;
...
let get t doc =
  ...
  let v =
    try Weak.get file.file_cache pos
    with e ->
      lprintf_nl "Exception %s for doc at pos %d (doc %d, combine %d)"
        (Printexc2.to_string e) pos doc combine;
      raise e
  in
  match v with
  | None -> (
      ...
      if verbose then
        lprintf_nl "REALLY READ FROM %d POS %d LEN %d" chunk_size pos len;
      let str = file_retrieve file pos in
      try Marshal.from_string str 0
      with e ->
        lprintf_nl "Marshal.from_string error";
        raise e )
  | Some v ->
      if verbose then lprintf_nl "Reply found in cache";
      v
...
let set_attrib t doc bool =
  if bool then t.store_all_doc.(doc) <- t.store_all_doc.(doc) lor attrib
  else
    let bin = t.store_all_doc.(doc) in
    t.store_all_doc.(doc) <- bin land lnot attrib

let get_attrib t doc = t.store_all_doc.(doc) land attrib <> 0
