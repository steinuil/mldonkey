...
let facility_of_string s =
  ...
  | "local7" -> `LOG_LOCAL7
  | invalid ->
      raise (Syslog_error ("facility_of_string: invalid facility, " ^ invalid))
...
let openlog
    ?(logpath =
      try
        ignore (Unix.stat "/dev/log");
        "/dev/log"
      with Unix.Unix_error (Unix.ENOENT, _, _) -> (
        try
          ignore (Unix.stat "/var/run/syslog");
          "/var/run/syslog"
        with Unix.Unix_error (Unix.ENOENT, _, _) -> "" ))
    ?(facility = `LOG_USER) ?(flags = []) ident =
  let loginfo =
    {
      fd = Unix.stderr;
      connected = false;
