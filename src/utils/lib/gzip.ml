...
let open_in ic =
  ...
  ( try
      ...
      if flags land 0xE0 <> 0 then raise (Error "bad flags, not a gzip file");
      for i = 1 to 6 do
      ...
      if flags land 0x08 <> 0 then
        while (* Skip original file name *)
              IO.read_byte ic <> 0 do
          ()
        done;
...
let open_in_file filename =
  ...
  try open_in (IO.input_channel ic)
  with e ->
...
let read_byte iz =
  if iz.in_avail = 0 then (
    let n = IO.input iz.in_chan iz.in_buffer 0 (String.length iz.in_buffer) in
    iz.in_pos <- 0;
    iz.in_avail <- n );
...
let rec input iz buf pos len =
  ...
  if iz.in_eof then 0
    ...
    if iz.in_avail = 0 then (
      let n =
        try IO.input iz.in_chan iz.in_buffer 0 (String.length iz.in_buffer)
    ...
    let finished, used_in, used_out =
      try
        Zlib.inflate iz.in_stream iz.in_buffer iz.in_pos iz.in_avail buf pos len
          Zlib.Z_SYNC_FLUSH
    ...
    if finished then
      try
        ...
        if iz.in_crc <> crc then raise (Error "CRC mismatch, data corrupted");
        if iz.in_size <> size then raise (Error "size mismatch, data corrupted");
        iz.in_eof <- true;
        used_out
      with IO.No_more_input -> raise (Error "truncated stream")
    else if used_out = 0 then input iz buf pos len
    else used_out )
...
let input_char iz =
  if input iz iz.char_buffer 0 1 = 0 then raise End_of_file
  else iz.char_buffer.[0]
...
let open_out ?(level = 6) oc =
  ...
  IO.write_byte oc 0x1F;
  (* ID1 *)
  IO.write_byte oc 0x8B;
  (* ID2 *)
  IO.write_byte oc 8;
  (* compression method *)
  IO.write_byte oc 0;
...
let open_out_file ?level filename =
  ...
  try open_out ?level (IO.output_channel oc)
  with exn ->
...
let rec output oz buf pos len =
  ...
  if oz.out_avail = 0 then (
    ignore (IO.really_output oz.out_chan oz.out_buffer 0 oz.out_pos);
    oz.out_pos <- 0;
    oz.out_avail <- String.length oz.out_buffer );
  let _, used_in, used_out =
    try
      Zlib.deflate oz.out_stream buf pos len oz.out_buffer oz.out_pos
        oz.out_avail Zlib.Z_NO_FLUSH
    with Zlib.Error (_, _) -> raise (Error "error during compression")
  in
...
let flush oz =
  let rec do_flush () =
    ...
    let finished, _, used_out =
      Zlib.deflate oz.out_stream oz.out_buffer 0 0 oz.out_buffer oz.out_pos
        oz.out_avail Zlib.Z_FINISH
    in
    oz.out_pos <- oz.out_pos + used_out;
    oz.out_avail <- oz.out_avail - used_out;
    if not finished then do_flush ()
  in
  do_flush ();
  (* Final data flush *)
...
let output_io io =
  ...
  IO.create_out ~write:(output_char oz)
    ~output:(fun s o l ->
      output oz s o l;
      l)
