...
let rec add x data = function
  ...
  | Node (l, v, d, r, h) ->
      if x = v then Node (l, x, data, r, h)
      else if x < v then bal (add x data l) v d r
      else bal l v d (add x data r)

let rec find x = function
  | Empty -> raise Not_found
  | Node (l, v, d, r, _) -> if x = v then d else find x (if x < v then l else r)

let rec mem x = function
  | Empty -> false
  | Node (l, v, _d, r, _) -> x = v || mem x (if x < v then l else r)
...
let rec remove x = function
  ...
  | Node (l, v, d, r, _h) ->
      ...
      else if x < v then bal (remove x l) v d r
...
let rec length_aux len = function
  | Empty -> len
  | Node (l, _v, _d, r, _) -> length_aux (length_aux (1 + len) l) r

let length map = length_aux 0 map

let top = function Empty -> raise Not_found | Node (_l, v, d, _r, _) -> (v, d)
...
let rec infix_nth map res =
  ...
  | (_count, None), Node (l, _v, d, r, _) ->
      infix_nth r
        ( match infix_nth l res with
        | (_count, Some _) as res -> res
        | count, None -> if count = 0 then (count, Some d) else (count - 1, None)
        )
