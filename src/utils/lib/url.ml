...
let encode s =
  ...
  Bytes.sub res 0 !pos |> Bytes.to_string
...
let to_string url =
  ...
  | ("http" | "ftp" | "ssh"), _ ->
      add_char res ':';
      add_string res (string_of_int url.port)
  | _, port when port <> 0 ->
      add_char res ':';
      add_string res (string_of_int url.port)
  | _ -> () );
...
let put_args s args =
  if args = [] then s
    ...
    let rec manage_args = function
      | [] -> assert false
      | [ (a, "") ] -> Buffer.add_string res (encode a)
      | [ (a, b) ] ->
          Buffer.add_string res (encode a);
          Buffer.add_char res '=';
          Buffer.add_string res (encode b)
      | (a, b) :: l ->
          Buffer.add_string res (encode a);
          Buffer.add_char res '=';
          Buffer.add_string res (encode b);
...
let of_string ?(args = []) s =
  let remove_leading_slashes s =
    let len = String.length s in
    let left =
      let rec aux i = if i < len && s.[i] = '/' then aux (i + 1) else i in
      aux 0
    in
    if left = 0 then s else String.sub s left (len - left)
  in
    ...
    let cut init_pos default_port =
      ...
      let user, pass, host, port, end_pos =
        if s.[new_pos] = '@' then
          ...
          let port =
            if port_str = "" then default_port else int_of_string port_str
          in
      ...
      (host, port, file, user, pass)
    in
    try
      let colon = String.index s ':' in
      let len = String.length s in
      if len > colon + 2 && s.[colon + 1] = '/' && s.[colon + 2] = '/' then
        let proto = String.sub s 0 colon in
        let port =
          match proto with "http" -> 80 | "ftp" -> 21 | "ssh" -> 22 | _ -> 0
        in
