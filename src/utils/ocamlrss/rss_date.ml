...
let tokens_of_string str =
  let rec scan_any (strm__ : _ Stream.t) =
    ...
    | Some ('0' .. '9' as c) ->
        Stream.junk strm__;
        scan_number (int_of_char c - 48) strm__
    | Some (('a' .. 'z' | 'A' .. 'Z') as c) ->
        Stream.junk strm__;
        let rest = strm__ in
  ...
  and scan_number a (strm__ : _ Stream.t) =
    ...
    | Some ('0' .. '9' as c) ->
        Stream.junk strm__;
        scan_number ((a * 10) + (int_of_char c - 48)) strm__
    | _ ->
        let rest = strm__ in
  ...
  and scan_word b (strm__ : _ Stream.t) =
    ...
    | _ ->
        let rest = strm__ in
        Stream.lcons
          (fun _ -> find_token (Buffer.contents b))
  ...
  and scan_comment n (strm__ : _ Stream.t) =
    ...
    | Some '(' ->
        Stream.junk strm__;
        scan_comment (n + 1) strm__
    | Some _ ->
        Stream.junk strm__;
        scan_comment n strm__
...
let parse str =
  ...
  let add_data ?h ?m ?s ?mdn ?tz ?dst ?wd ?md ?mo ?y () =
    ...
    in
    let tz =
      match (tz, dst) with Some tz, Some true -> Some (tz - 100) | _ -> tz
    in
    let tz =
      match tz with
      | None -> None
  ...
  let rec scan_gen (strm__ : _ Stream.t) =
    ...
    | Some (Number n) ->
        Stream.junk strm__;
        scan_number n strm__
    | Some (Zone tz) ->
        Stream.junk strm__;
        let dst =
          try scan_dst strm__ with Stream.Failure -> raise (Stream.Error "")
        in
        let rest = strm__ in
        add_data ~tz ?dst ();
        scan_gen rest
    ...
    | Some (Month mo) -> (
        ...
        | Some (Number md) ->
            Stream.junk strm__;
            scan_date_m mo md strm__
        | _ -> raise (Stream.Error "") )
    | Some _ ->
        Stream.junk strm__;
        invalid_arg "Parse.date"
    | _ -> ()
  and scan_number n (strm__ : _ Stream.t) =
    ...
    | Some Slash -> (
        ...
        | Some (Number m) ->
            Stream.junk strm__;
            scan_date_s n m strm__
        | _ -> raise (Stream.Error "") )
    | Some Minus ->
        Stream.junk strm__;
        scan_date_d n strm__
    | Some (Month mo) ->
        Stream.junk strm__;
        let rest = strm__ in
        add_data ~md:n ~mo ();
  ...
  and scan_hour h m (strm__ : _ Stream.t) =
    ...
    | Some Colon -> (
        ...
        | Some (Number s) ->
            Stream.junk strm__;
            scan_hour_second h m s strm__
        | _ -> raise (Stream.Error "") )
    | _ -> (
        match try Some (scan_tz strm__) with Stream.Failure -> None with
        | Some tz ->
            let rest = strm__ in
            add_data ~h ~m ~tz ();
            scan_gen rest
  ...
  let may_get r =
    match !r with None -> invalid_arg "Parse.date" | Some r -> r
  in
...
let months_start = [| 0; 31; 59; 90; 120; 151; 181; 212; 243; 273; 304; 334 |]
...
let create ?(zone = 0) time =
  ...
  let n100, r100 =
    ...
    else ((r400 - 1) / 36524, (r400 - 1) mod 36524)
  in
  let n4, r4 =
  ...
  let minute = minutes mod 60 and hour = minutes / 60 in
  {
    year;
    month = month + 1;
...
let mk_mail_date ?zone t =
  format ~fmt:"%a, %d %b %Y %H:%M:%S %z" (create ?zone t)

let mk_usenet_date ?zone t =
  format ~fmt:"%A, %d-%b-%y %H:%M:%S %z" (create ?zone t)

let mk_timezone t = format ~fmt:"%z" (create ~zone:localzone t)
