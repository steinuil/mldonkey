...
let get_att ?(required = true) atts name =
  ...
  with Not_found -> if required then raise Not_found else ""

let get_opt_att atts name =
  let name = String.lowercase_ascii name in
  try
    Some (snd (List.find (fun (s, _) -> String.lowercase_ascii s = name) atts))
  with Not_found -> None
...
let get_source xmls =
  try
    ...
    | Element (_, atts, [ PCData s ]) ->
        Some { src_name = s; src_url = get_att atts "url" }
    | _ -> None
  with Not_found -> None
  ...
  let f acc = function
    | Element (tag, atts, [ PCData s ])
      when String.lowercase_ascii tag = "category" ->
        { cat_name = s; cat_domain = get_opt_att atts "domain" } :: acc
    | _ -> acc
  in
...
let item_of_xmls xmls =
  ...
  let date =
    ...
    | None -> None
    | Some s -> ( try Some (Rss_date.parse s) with _ -> None )
  in
  {
    item_title = f "title";
    item_link = f "link";
...
let channel_of_xmls xmls =
  let f s =
    try
      ...
      | Element (_, _, []) -> ""
      | _ -> raise Not_found
    with Not_found -> failwith ("Parse error: no correct " ^ s)
  in
  let f_opt s =
    try
      match List.find (find_ele s) xmls with
...
let channel_of_source source =
  ...
  | Element (e, _atts, subs) -> (
      ...
      | "rss" -> (
          ...
          | _ -> failwith "Parse error: two much things in rss" )
      | "rdf:rdf" -> (
          match subs with
          | [] -> failwith "Parse error: no channel"
          | Element (e, _atts, subs) :: q -> (
              match String.lowercase_ascii e with
...
let channel_of_string s = channel_of_source (XmlParser.SString s)

let channel_of_file file = channel_of_source (XmlParser.SFile file)

(** {2 Printing} *)

let opt_element opt s =
  match opt with None -> [] | Some v -> [ Element (s, [], [ PCData v ]) ]
...
let xml_of_category c =
  let atts =
    match c.cat_domain with None -> [] | Some d -> [ ("domain", d) ]
  in
...
let xml_of_source s =
  Element ("source", [ ("url", s.src_url) ], [ PCData s.src_name ])
  ...
  let xml_ch =
    Element
      ( "channel",
        [],
        [
          f ch.ch_title "title"; f ch.ch_link "link"; f ch.ch_desc "description";
        ]
        @ List.flatten
            [
