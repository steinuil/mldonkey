...
let create_in ~read ~input ~close =
  { in_read = read; in_input = input; in_close = close }
...
let input i s p l =
  ...
  if l = 0 then 0 else i.in_input s p l
...
let input_channel ch =
  {
    in_read =
      (fun () -> try input_char ch with End_of_file -> raise No_more_input);
    in_input =
      (fun s p l ->
...
let pipe () =
  ...
  let input s p l =
    ...
    r
  in
  let write c = Buffer.add_char output c in
  let output s p l =
    Buffer.add_subbytes output s p l;
    l
  in
  let input = { in_read = read; in_input = input; in_close = (fun () -> ()) } in
  let output =
    {
      out_write = write;
      out_output = output;
...
type 'a bc = { ch : 'a; mutable nbits : int; mutable bits : int }
...
let input_bits ch = { ch; nbits = 0; bits = 0 }

let output_bits ch = { ch = cast_output ch; nbits = 0; bits = 0 }
...
let drop_bits b = b.nbits <- 0

let rec write_bits b ~nbits x =
  let n = nbits in
  if n + b.nbits >= 32 then (
    if n > 31 then raise Bits_error;
...
let flush_bits b = if b.nbits > 0 then write_bits b ~nbits:(8 - b.nbits) 0
...
let from_in_channel ch =
  ...
  in
  let input s p l = ch#input s p l in
  create_in ~read ~input ~close:ch#close_in
...
let from_out_channel ch =
  ...
  in
  let output s p l = ch#output s p l in
  create_out ~write ~output ~flush:ch#flush ~close:ch#close_out
...
let from_in_chars ch =
  let input s p l =
    ...
    try
      ...
      l
    with End_of_file when !i > 0 -> !i
  in
  create_in ~read:ch#get ~input ~close:ch#close_in
...
let from_out_chars ch =
  let output s p l =
    ...
    l
  in
  create_out ~write:ch#put ~output ~flush:ch#flush ~close:ch#close_out
