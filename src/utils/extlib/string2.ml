  ...
  let rec iter pos =
    try
      if pos = len then [ "" ]
        ...
        if pos2 = pos then "" :: iter (pos + 1)
        else String.sub s pos (pos2 - pos) :: iter (pos2 + 1)
...
let rec unsplit l c =
  ...
  | [] -> ""
  | [ x ] -> x
  | x :: (_ :: _ as tail) -> Printf.sprintf "%s%c%s" x c (unsplit tail c)
...
let words s =
  ...
  and iter_in pos0 pos =
    if pos = len then [ String.sub s pos0 (len - pos0) ]
    else
      let c = s.[pos] in
      match c with
      | ' ' | '\009' | '\010' | '\013' ->
          String.sub s pos0 (pos - pos0) :: iter_out (pos + 1)
...
let upp_initial = String.uppercase_ascii
...
let subequal s1 pos1 s2 pos2 len =
  ...
  &&
  let rec iter pos =
    pos = len || (s1.[pos1 + pos] = s2.[pos2 + pos] && iter (pos + 1))
  in
  iter 0
...
let subcontains s sub =
  ...
  let rec after_pos pos =
    (not (pos + slen > len))
    && (subequal s pos sub 0 slen || after_pos (pos + 1))
  in
  after_pos 0
...
let tokens s =
  ...
  let rec iter_space start_pos i =
    if i = len then [ String.sub s start_pos (i - start_pos) ]
    else
      let c = s.[i] in
      if is_space c then
        String.sub s start_pos (i - start_pos) :: iter_next (i + 1)
      else iter_space start_pos (i + 1)
  and iter_next i =
    if i = len then []
    else
      let c = s.[i] in
      if is_space c then iter_next (i + 1)
      else if c = '\'' || c = '"' then iter_delim (i + 1) (i + 1) c
      else iter_space i (i + 1)
  and iter_delim start_pos i delim =
    if i = len then failwith (Printf.sprintf "Unterminated delim %c" delim)
    else
      let c = s.[i] in
      if c = delim then
        String.sub s start_pos (i - start_pos) :: iter_next (i + 1)
      else if c = '\\' && i + 1 < len then iter_delim start_pos (i + 2) delim
      else iter_delim start_pos (i + 1) delim
  in
...
let stem s =
  ...
  for i = 0 to Bytes.length s - 1 do
    ...
    match c with 'a' .. 'z' | '0' .. '9' -> () | _ -> Bytes.set s i ' '
  done;
...
let dehtmlize =
  ...
  fun s ->
    s |> Str.global_replace br_regexp "\n" |> Str.global_replace tag_regexp ""
