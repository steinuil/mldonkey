...
let string_to_path sep str =
  ...
  let rec iter start pos =
    ...
    else if str.[pos] = sep then
      if pos <= start then iter (pos + 1) (pos + 1)
      else String.sub str start (pos - start) :: iter (pos + 1) (pos + 1)
    else iter start (pos + 1)
  in
  iter 0 0

let path_to_string sep path =
  let s =
    List.fold_left (fun str dir -> Printf.sprintf "%s%c%s" str sep dir) "" path
  in
