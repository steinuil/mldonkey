...
let usage speclist errmsg =
  ...
  List.iter
    (function
      | key, _, doc ->
          eprintf "  %s %s\n"
            (if String.length key = 2 then key else "-" ^ key)
            doc)
    speclist;
...
let parse_argv argv speclist anonfun errmsg =
  ...
  let stop error =
    ...
    | Unknown "-help" -> ()
    | Unknown "--help" -> ()
    | Unknown s -> eprintf "%s: unknown option `%s'.\n" progname s
    | Missing (n, s) ->
    ...
    if error = Unknown "-help" || error = Unknown "--help" then exit 0
    else exit 2
  in
    ...
    if sl >= 2 && s.[0] = '-' && s.[1] = '-' then (
      ...
      ( try
          ...
          | Array (n, f) when n > 0 && !current + n < l ->
              ...
              f args;
              current := !current + n
          | Array (n, _) -> stop (Missing (l - !current - n, s))
          | Rest f ->
...
let parse2_argv argv speclist anonfun errmsg =
  ...
  while !current < l do
    ...
    let sl = String.length s in
    if sl >= 1 && s.[0] = '-' then (
      let action = try assoc3 s speclist with Not_found -> stop (Unknown s) in
      ( try
          match action with
          | Unit f -> f ()
          | Set r -> r := true
          ...
          | Array (n, f) when n > 0 && !current + n < l ->
              ...
              f args;
              current := !current + n
          | Array (n, _) -> stop (Missing (n - (l - !current) + 1, s))
          | Rest f ->
              while !current < l - 1 do
                f argv.(!current + 1);
                incr current
