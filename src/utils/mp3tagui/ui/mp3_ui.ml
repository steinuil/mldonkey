...
let params_from_tag tag =
  let title =
    C.string ~f:(fun s -> tag.title <- s) Mp3_messages.title tag.title
  in
  let artist =
    C.string ~f:(fun s -> tag.artist <- s) Mp3_messages.artist tag.artist
  in
  let album =
    C.string ~f:(fun s -> tag.album <- s) Mp3_messages.album tag.album
  in
  let year = C.string ~f:(fun s -> tag.year <- s) Mp3_messages.year tag.year in
  let tracknum =
    C.string
      ~f:(fun s -> tag.tracknum <- (try int_of_string s with _ -> 0))
      Mp3_messages.tracknum
      (string_of_int tag.tracknum)
  in
  let comment =
    C.string ~f:(fun s -> tag.comment <- s) Mp3_messages.comment tag.comment
  in
  let genre =
    C.combo
      ~f:(fun s -> tag.genre <- Mp3_misc.genre_of_string s)
      ~blank_allowed:true Mp3_messages.genre Mp3_genres.genres_names
      (Mp3_misc.string_of_genre tag.genre)
  in
  let params = [ artist; album; title; comment; tracknum; year; genre ] in
  params
