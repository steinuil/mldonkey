...
module Id3v1 = struct
  ...
  let read_channel ic =
    ...
    let readstring len =
      ...
      Mp3_misc.chop_whitespace (Bytes.to_string buf) 0
    in
  ...
  (** Write the given tag info into the given file. 
    ...
    *)
  let write ?(title = "") ?(artist = "") ?(album = "") ?(year = "")
      ?(comment = "") ?(tracknum = 1) ?(genre = 0) filename =
    let oc =
      open_out_gen [ Open_wronly; Open_append; Open_binary ] 0o666 filename
    in
    let put s len =
      let l = String.length s in
      for i = 0 to min l len - 1 do
        output_char oc s.[i]
      done;
      for _ = l to len - 1 do
        output_byte oc 0
  ...
  let write_tag filename tag =
    write ~title:tag.title ~artist:tag.artist ~album:tag.album ~year:tag.year
      ~comment:tag.comment ~tracknum:tag.tracknum ~genre:tag.genre filename
...
module Id3v2 = struct
  ...
  let length_header header =
    (Char.code header.[6] lsl 21)
    lor (Char.code header.[7] lsl 14)
    lor (Char.code header.[8] lsl 7)
    lor Char.code header.[9]
  ...
  let read_channel ic =
    try
      ...
      if not (valid_header (Bytes.to_string header)) then raise Not_found;
      let len = length_header (Bytes.to_string header) in
      ...
      while pos_in ic < startpos + len do
        let frameid = input_buffer ic 4 |> Bytes.to_string in
        let framelen = input_int4 ic in
        let flags1 = input_byte ic in
        let flags2 = input_byte ic in
        let framedata = input_buffer ic framelen |> Bytes.to_string in
        if flags1 land 0b00011111 = 0 && flags2 = 0 then
          try tags := (frameid, decode_framedata frameid framedata) :: !tags
          with Not_found -> ()
      done;
      List.rev !tags
    with x -> raise x
  ...
  let output_byte oc b =
    if !last_byte_written = 0xFF then
      if b = 0 || b land 0b11100000 = 0b11100000 then Stdlib.output_byte oc 0;
  ...
  let output_frame oc (name, data) =
    ...
    if name = "TXXX" then (
      ...
      (* tag *)
      output_int4 oc (String.length data + 10);
      (* length *)
      output_byte oc 0;
      output_byte oc 0;
      (* null flags *)
      ...
      output_string oc name;
      (* tag *)
      output_int4 oc (String.length data + 2);
      (* length *)
      output_byte oc 0;
      output_byte oc 0;
      (* null flags *)
      ...
      output_string oc name;
      (* tag *)
      output_int4 oc (String.length data);
      (* length *)
      output_byte oc 0;
      output_byte oc 0;
      (* null flags *)
  ...
  let append_data oc filename =
    ...
    try
      ( try
          ...
          really_input ic header 0 10;
          if not (valid_header (Bytes.to_string header)) then raise Not_found;
          seek_in ic (pos_in ic + length_header (Bytes.to_string header))
        with Not_found | End_of_file -> seek_in ic 0 );
    ...
    try
      ...
      ( try
        ...
        with x ->
          ...
          raise x );
      close_out oc;
      match srcname with None -> Sys.remove origname | Some _ -> ()
    with x ->
      (match srcname with None -> Sys.rename origname filename | Some _ -> ());
      raise x
...
let v2_to_v1 tags =
  {
    Id3v1.title = (try List.assoc "TIT2" tags with Not_found -> "");
    Id3v1.artist =
      ( try List.assoc "TPE1" tags
        with Not_found -> ( try List.assoc "TPE2" tags with Not_found -> "" ) );
    Id3v1.album = (try List.assoc "TALB" tags with Not_found -> "");
    Id3v1.year = (try List.assoc "TYEA" tags with Not_found -> "");
    Id3v1.comment = (try List.assoc "TXXX" tags with Not_found -> "");
    Id3v1.tracknum =
      ( try int_of_string (List.assoc "TRCK" tags)
        with Not_found | Failure _ -> 0 );
    Id3v1.genre =
      ( try
          let g = List.assoc "TCON" tags in
...
let v1_to_v2 t =
  ...
  if t.Id3v1.comment <> "" then tags := ("TXXX", t.Id3v1.comment) :: !tags;
  if t.Id3v1.year <> "" && t.Id3v1.year <> "0" then
    tags := ("TYEA", t.Id3v1.year) :: !tags;
  if t.Id3v1.album <> "" then tags := ("TALB", t.Id3v1.album) :: !tags;
  if t.Id3v1.artist <> "" then tags := ("TPE1", t.Id3v1.artist) :: !tags;
  if t.Id3v1.title <> "" then tags := ("TIT2", t.Id3v1.title) :: !tags;
  !tags

let read_channel_both_v1 ic =
  let t2 =
    try v2_to_v1 (Id3v2.read_channel ic) with Not_found -> Id3v1.no_tag
  in
  let t1 = try Id3v1.read_channel ic with Not_found -> Id3v1.no_tag in
  Id3v1.merge t2 t1

let read_channel_both_v2 ic =
  let t2 = try Id3v2.read_channel ic with Not_found -> Id3v2.no_tag in
  let t1 =
    try v1_to_v2 (Id3v1.read_channel ic) with Not_found -> Id3v2.no_tag
  in
  Id3v2.merge t2 t1
